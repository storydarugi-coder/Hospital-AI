/**
 * GPT-5.2 프롬프트 시스템 v6.1
 * 
 * v6.1 구어체/자연스러움/의료 용어 강화:
 * - 구어체 리듬: ~하나요?/~해요/~당기지 않나요? 필수 사용
 * - 의료 용어 완전 금지: 의료/의료진/전문/전문가/전문적인 절대 금지
 * - 마지막 소제목 문단 2개 강제 (HTML <p> 태그로 검증)
 * - 시즌/날씨 키워드 필수 포함 (1월/겨울/차가운 공기 등)
 * 
...
[P1] 🚨 절대 준수 (즉시 탈락):
  ...
  ✓ 🔴 마지막 소제목 아래 2개 문단 초과 금지 (3개 이상 = 탈락)
  ✓ 🚨 불안 자극 표현 절대 금지
  ✓ 🚨 단정적 인과관계 절대 금지
  ✓ 🚨 적극적 관리 조언 절대 금지
...
[P2] ⚠️ 반드시 준수:
  ...
  🚨 마지막 소제목 문단 제한 (CRITICAL - 절대 준수):
  ✓ 마지막 소제목 아래는 반드시 2개 문단 이하
  ✓ 3개 이상 문단 작성 시 즉시 탈락
    • 1문단: 상황 설명
    • 2문단: 마무리 (선택적 안내)
    ⚠️ 절대로 3문단 이상 작성 금지!
...
5️⃣ 마지막 소제목 작성 (🚨 CRITICAL - 문단 2개 초과 금지):
  
  🔴 필수 규칙:
  ✓ 마지막 소제목 아래는 반드시 2개 문단 이하
  ✓ 절대로 3문단 이상 작성 금지 (즉시 탈락)
...
8. □ 소제목 개수 및 형태
   ...
   🚨 CRITICAL: 마지막 <h2> 태그 이후 문단 개수 세기
   → <p> 태그 개수: ___개 (2개 이하여야 함!)
   → 3개 이상 발견 시 즉시 축약/병합 (절대 준수)
