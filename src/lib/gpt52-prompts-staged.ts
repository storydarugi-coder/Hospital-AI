...
[절대 생성 금지 - P0 최우선!]
1. 🚫 질환명/병명/증후군/의학 용어 금지 (SEO 키워드 제외)
   ❌ ○○증, ○○병, ○○근종, ○○내막증, 이차성 ○○, ○○염, ○○증후군
   ❌ 단, 사용자가 입력한 SEO 키워드(예: "자궁근종")는 허용
   ✅ 키워드 외의 질환명은 절대 추가하지 말 것!
...
[P2] ⚠️ 반드시 준수:
  🚨🚨🚨 [글자 수 제한 - 최우선 규칙! P1급 중요도!] 🚨🚨🚨
  ✓ 본문 분량: 공백 제외 **정확히 ${textLength}자** (허용 오차 ±50자만!)
  
  ⚠️ 현재 요청된 목표 글자 수: ${textLength}자
  ✅ 허용 범위: ${textLength - 50}자 ~ ${textLength + 50}자
  ❌ ${textLength + 100}자 이상 = 즉시 탈락! (글이 2배로 길어지면 안 됨!)
  
  🔴 왜 글자 수를 지켜야 하나요?
  - ${textLength}자를 요청했는데 3000자 넘게 쓰면 독자가 이탈합니다
  - 글이 너무 길면 핵심이 묻힙니다
  - 요청한 분량을 정확히 지키는 것이 프로의 기본입니다
  
  🔥 작성 팁:
  - 목표보다 100자 적게 쓰는 게 안전합니다 (${textLength - 100}자 ~ ${textLength}자)
  - 소제목당 문단 2개, 문단당 2~3문장이면 적당합니다
  - 각 문장 40자 이내로 짧게!
  - 중복 내용, 뻔한 설명은 과감히 삭제!
  
  📏 작성 후 반드시 글자 수 체크:
  - HTML 태그 제외, 공백 제외 후 실제 글자 수 세기
  - ${textLength + 50}자 초과 시 → 불필요한 문장 삭제
  - ${textLength - 50}자 미만 시 → 구체적 설명 추가
