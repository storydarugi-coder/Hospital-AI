...
2️⃣ 구체적인 생활 장면:
  ❌ "운동이 도움이 됩니다"
  ✅ "양치하면서 한쪽 다리씩 들어보세요. 균형 잡는 것만으로도 무릎 주변 근육이 깨어납니다"

3️⃣ 흔하지 않은 연결고리:
  ❌ "스트레스가 위장에 영향"
  ✅ "회의 전 배가 아픈 건 장이 긴장을 먼저 느끼기 때문입니다"

4️⃣ 예상 밖의 디테일:
  ❌ "수면이 중요합니다"
  ✅ "베개에 침이 묻어 있다면 입으로 숨 쉬었다는 신호입니다"

5️⃣ 소제목 차별화:
  ❌ "무릎 통증의 원인" / "증상과 특징"
  ✅ "계단 내려갈 때만 아픈 이유" / "쪼그려 앉았다 일어서기 힘들다면"

⚠️ 체크:
□ 다른 블로그에서 흔한 표현 피했는가?
□ 교과서 같은 정보 나열 피했는가?
□ "이런 걸 알려주는 곳은 여기밖에 없네" 싶은 디테일이 있는가?
...
🔴 강압적 톤 금지:
  방치하면 위험 → ✅ "경과를 지켜보는 것도 방법이에요"
  ~해야 합니다 → ✅ "~도움이 될 수 있어요"
  검사 받으세요 → ✅ "확인이 도움될 수 있어요"
  병원 가세요 → ✅ "상담을 받아보는 것도 좋아요"
...
위반 예시:
  ❌ 심각: "고혈압은 뇌졸중을 유발합니다"
  ❌ 중간: "고혈압은 뇌졸중으로 이어집니다"
  ❌ 약함: "고혈압은 뇌졸중과 관련이 있습니다" (이것도 금지!)
  ✅ 안전: "뇌혈관 건강과의 연관성이 언급되기도 합니다"
...
❌ 중복 예시:
  "혈당이 오르내리면 피로를 느낄 수 있습니다. 
   혈당 변화가 생기면 몸이 무거워집니다."
  → 같은 내용 반복! (혈당 변화 → 피로/무거움)

✅ 올바른 예시:
  "혈당이 오르내리면 피로를 느낄 수 있습니다. 
   이런 상태가 지속되면 집중력 저하로 이어지기도 합니다."
  → 원인-결과 구조로 진행
...
❌ 만연체 예시:
  "무릎 통증은 관절 연골이 손상되면서 나타나는 경우가 많은데, 
   특히 계단을 오르내릴 때나 쪼그려 앉을 때 더 심해지며, 
   이러한 증상이 반복되면 일상생활에도 불편함을 느끼게 되고, 
   방치할 경우 걷기조차 힘들어질 수 있습니다."
  → 4개 절이 연결된 만연체!
...
🎯 SEO 키워드 (사용자가 입력한 키워드):
  • **필수**: 본문 전체에서 최소 1회 ~ 최대 3회 자연스럽게 포함
...
[P2] ⚠️ 반드시 준수:
  ✓ 본문 분량: ${textLength}~${textLength + 100}자 (엄격)
...
7. □ 분량 확인
   → 현재 글자 수: ____자
   → 목표 범위: ${textLength}~${textLength + 100}자
   → 범위 내인가? (부족/초과 시 조정)
