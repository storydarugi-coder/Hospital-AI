...
━━━━━━━━━━━━━━━━━━
❌ 읽기만 하는 체크리스트:
   "누웠다 일어날 때 허리가 펴지지 않는지 살펴볼 필요가 있습니다"
✅ 실제로 해보게 만드는 명령형:
   "지금 앉아 계시다면, 천천히 일어나보세요. 허리를 펴는 순간 '억' 하는 느낌이 든다면 단순 근육통과는 다른 양상일 수 있습니다."

🎯 작성 원칙:
- "지금", "한번", "천천히" 등 행동 유도어 사용
- "~해보세요", "~해볼까요" 등 명령형/청유형 종결
- 구체적 동작 묘사 ("팔을 천천히 올려보세요", "고개를 좌우로 돌려보세요")
- 감각 표현 포함 ("'억' 하는 느낌", "뻐근한 느낌", "당기는 느낌")
...
🔍 출처 검증 방법
- URL 실제 접근 테스트 필수
- 구체적 페이지 경로 확인 (메인 페이지 ≠ 출처)
- 필요 시 웹 검색으로 재확인
- 불확실하면 출처 명시 생략 ("최근 연구에 따르면"으로 일반화)

💡 권장 작성 방식
- 특정 출처 필요 없는 일반적 정보는 "알려진 바에 따르면" 사용
- 구체적 통계/수치는 반드시 신뢰 가능한 출처 필요
- 출처 불확실 시: 정보 자체를 제외하거나 일반화`;
...
{
  "t": "어깨 뻣뻣할 때 확인법",
  "c": "<p>패딩 소매에 팔을 넣다가 걸리는 느낌이 반복되는 경우가 있다. 특히 아침에 일어났을 때 어깨가 굳어 있는 듯한 느낌이 드는 분들이 있다.</p><h3>지금 바로 해볼 수 있는 확인법</h3><p>지금 팔을 천천히 올려보세요. 귀 옆까지 올라가는지 확인해볼까요. 중간에 '뚝' 소리가 나거나 어느 지점에서 더 이상 올라가지 않는다면 한번 살펴볼 만하다. 반대쪽 팔과 비교했을 때 차이가 느껴진다면 더욱 그렇다.</p><h3>밤에 유독 더 불편한 이유</h3><p>야간에 옆으로 돌아눕다가 깨어나는 경험을 하는 분들이 있다. 어깨에 체중이 실리면서 통증이 느껴지는 경우도 있으며, 개인차가 있으므로 경과를 살펴보는 것이 중요하다.</p><h3>이런 동작이 불편하다면</h3><p>헤어드라이어를 들어 올려보세요. 팔이 무거워지는 느낌이 든다면 체크가 필요하다. 뒷주머니에서 지갑을 꺼낼 때도 마찬가지다. 증상이 지속된다면 전문적인 확인을 받아보는 것도 방법이다.</p>",
