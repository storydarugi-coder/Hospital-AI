...
[P2] ⚠️ 반드시 준수:
  ...
  📐 **글자 수 계산 방법 (정확히 이렇게 세세요!)**:
  
  1️⃣ HTML 태그 완전 제거:
     ❌ 잘못된 예: "<p>안녕하세요</p>" → 15자로 계산 (틀림!)
     ✅ 올바른 예: "<p>안녕하세요</p>" → "안녕하세요" = 6자 (맞음!)
     
  2️⃣ 공백 제거:
     ❌ 잘못된 예: "안녕 하세요" → 7자로 계산 (틀림!)
     ✅ 올바른 예: "안녕 하세요" → "안녕하세요" = 6자 (맞음!)
     
  3️⃣ 순수 텍스트만 카운트:
     예시)
     원본: <p>목이 뻐근할 수 있습니다.</p><p>며칠째 반복되기도 합니다.</p>
     계산: "목이뻐근할수있습니다며칠째반복되기도합니다" = 27자
  
  🎯 **목표 글자 수**: 정확히 ${textLength}자 (공백 제외, HTML 태그 제외)
  📏 **허용 범위**: ${textLength - 50}자 ~ ${textLength + 50}자 (이 범위 안에서만!)
  
  🔴 **절대 금지**:
  - ${textLength + 100}자 이상 = 즉시 실격! (너무 김!)
  - ${textLength - 200}자 미만 = 즉시 실격! (너무 짧음!)
  - HTML 태그 포함해서 세는 것 = 완전 틀림!
  - 공백 포함해서 세는 것 = 완전 틀림!
  
  💡 **왜 정확히 세야 하나요?**
  - 사용자가 ${textLength}자 요청 → 딱 그 근처로 맞춰야 함!
  - 980자 쓰고 1900자라고 착각하면 안 됨!
  - HTML 태그는 글자 수에 포함 안 됨!
  ...
  - 문단당 문장 2~3개
  - 문장당 30~40자
  - 이렇게 하면 약 1500~1800자 나옴
  
  📐 **작성 후 반드시 체크**:
  1. HTML 태그 전부 제거 (<p>, <h2>, </p>, </h2> 다 삭제)
  2. 순수 텍스트만 추출
  3. 공백 전부 제거
  4. 남은 글자 수 세기
  5. ${textLength - 50}자 ~ ${textLength + 50}자 범위인지 확인
  6. 범위 벗어나면 문장 추가/삭제로 조정
  
  🔥 **실전 예시**:
  잘못된 계산:
  "<p>목이 뻐근합니다.</p>" = 16자로 계산 ❌
  
  올바른 계산:
  "<p>목이 뻐근합니다.</p>" → "목이뻐근합니다" = 8자 ✅
...
7. □ 🚨🚨🚨 분량 확인 (공백 제외, HTML 태그 제외) - 최우선!
   
   📐 **정확한 글자 수 계산 방법**:
   
   STEP 1: HTML에서 순수 텍스트만 추출
   - <p>, <h2>, </p>, </h2> 등 모든 태그 제거
   - 남은 텍스트만 가져오기
   ...
   📏 범위: ${textLength - 50}자 ~ ${textLength + 50}자
   
   ❌ 잘못된 계산 예시:
   "<p>목이 뻐근합니다.</p><p>며칠째입니다.</p>"
   → 41자로 계산 ❌ (HTML 태그 포함해서 셈)
   ...
   3. 남은 텍스트에서 공백 삭제
   4. 글자 수 세기
   5. ${textLength - 50} ~ ${textLength + 50} 범위인지 확인
   6. 벗어나면 문장 추가/삭제로 조정
   
   ⚠️ 주의: HTML 태그를 포함해서 세면 안 됨!
   ⚠️ 주의: 공백을 포함해서 세면 안 됨!
