...
[🚨 검색 규칙 - health.kdca.go.kr 전용 검색 강화!]

**⚠️⚠️⚠️ 핵심 원칙: health.kdca.go.kr 전용 검색을 최우선으로! ⚠️⚠️⚠️**
...
🥇 **1순위 (최우선! 반드시 먼저 검색!)**:
   ...
   → 검색어에 "site:health.kdca.go.kr" 추가하여 검색!
   → 🚨 이 사이트에서 충분한 자료를 찾았다면 해외 사이트 검색 생략!
   → 🚨 health.kdca.go.kr 결과를 가장 많이 수집!
   → URL 형식: https://health.kdca.go.kr/...

🥈 **2순위**:
   ✅ kdca.go.kr (질병관리청 공식 - 보도자료, 통계, 감염병)
   → 검색어에 "site:kdca.go.kr" 추가하여 검색!

🥉 **3순위: 대한민국 정부 기관 (국내 정보 우선!)**
✅ mohw.go.kr (보건복지부)
...
4️⃣ **4순위: 해외 공신력 기관 (국내 정보 부족 시에만!)**
✅ who.int (WHO - 세계보건기구)
✅ cdc.gov (CDC - 미국 질병통제예방센터)
✅ nih.gov (NIH - 미국 국립보건원)
✅ pubmed.ncbi.nlm.nih.gov (PubMed 논문)
✅ jamanetwork.com (JAMA), nejm.org (NEJM), thelancet.com (Lancet)
  → ⚠️ 1~3순위에서 충분한 자료를 찾았다면 이 단계는 생략!

**🔐 검색 전략 (health.kdca.go.kr 전용 강화!):**
1️⃣ 먼저 "주제 site:health.kdca.go.kr"로 검색 → 결과 최우선 수집!
2️⃣ 부족하면 "주제 site:kdca.go.kr"로 검색
3️⃣ 여전히 부족하면 다른 국내 정부기관 (.go.kr) 검색
4️⃣ 대한OO학회 (.or.kr) 검색
5️⃣ 마지막으로 해외 공신력 사이트 검색 (최소화!)
...
{
  "collected_facts": [{"fact": "수집한 사실 정보", "source": "출처 (학회/기관명)", "year": ${CURRENT_YEAR}, "url": "참고 URL (health.kdca.go.kr 우선)"}],
...
████████████████████████████████████████████████████████████████████████████████
[🚨 판단 단정형 글쓰기 금지 - 의료법 준수 필수!]
████████████████████████████████████████████████████████████████████████████████

**⚠️ 문제: 단정 표현("~입니다", "~됩니다")이 4회 이상 반복되면 의료법 위반 리스크!**
...
1. **"~입니다" 변환 공식:**
   ❌ "이 증상은 독감입니다"
   ✅ "이런 증상이 겹친다면 독감일 수 있어 주의가 필요해요"
   ✅ "이런 패턴이 보인다면 독감 가능성을 고려해볼 수 있습니다"

2. **"~됩니다" 변환 공식:**
   ❌ "치료가 필요합니다"
   ✅ "전문가의 도움을 받아보는 것도 방법이에요" (의료법 준수)
   ✅ "이 시점에서는 전문의 상담이 판단에 도움이 될 수 있습니다"

3. **"~것입니다" 변환 공식:**
   ❌ "병원을 방문해야 하는 것입니다"
   ✅ "몸이 보내는 신호를 좀 더 예민하게 봐야 할 때일 수 있어요"
   ✅ "이때가 컨디션 관리에 집중할 시점일 수 있습니다"
...
**📌 핵심 원칙: 단정하지 말고, "~수 있습니다", "~경우가 있습니다", "~로 알려져 있습니다" 사용!**
...
`;
  ...
  const blogPrompt = `
    ...
    ⚠️⚠️⚠️ **핵심: health.kdca.go.kr 전용 검색 강화!** ⚠️⚠️⚠️
