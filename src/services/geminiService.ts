...
[최우선 규칙 - 반드시 지켜야 할 것]
1. 이모지 완전 금지 - 본문/제목에 이모지 절대 사용하지 않음
2. 구체적 숫자 완전 금지 - 빈도, 기간, 비율, 통계 숫자 사용하지 않음
   - "연 2~4회" (X) → "드물지 않게" (O)
   - "200여 종" (X) → "다양한 원인" (O)
   - "70%" (X) → "상당수가" (O)
   - "24~48시간" (X) → "개인차가 있으며" (O)
3. 병의원 마케팅 전문가가 쓴 것처럼 깔끔하고 신뢰감 있는 톤 유지
[최우선 규칙 끝]
...
**[🚨 숫자 완전 금지 - 의료광고법 위반 방지!]**
❌ 모든 구체적 숫자 사용 금지:
- 빈도: "연 2~4회", "6~8회", "10명 중 8명"
- 기간: "24~48시간", "2~3일", "1주일"
- 비율: "70%", "200여 종", "절반 이상"
- 통계: "51.7%", "90% 이상"
✅ 대체 표현:
...
**0. 이모지 완전 금지**
   - 본문에 이모지 절대 사용하지 않음 (전문 의료 콘텐츠 톤)
   - 병의원 마케팅 달인이 쓴 것처럼 깔끔하고 신뢰감 있게
   - 강조는 이모지 대신 문장 구조와 단어 선택으로
...
[중요]
[[금지] 증상 나열 절대 금지 패턴 - 진단 기준형 서술 금지]
[중요]
...
**1.3. 구체적 연도 표기 제거 (최신성 확보)**

🚨🚨🚨 **절대 금지: 기관명(연도) 형식 사용 금지!** 🚨🚨🚨
...
[중요]
[🏷️ 소제목 필수 규칙 - 블로그탭 장기 노출용!]
[중요]

**⚠️ 블로그탭에 유리한 '사람 말투 소제목'을 작성하세요!**

**[절대 금지 - 의학 백과/병원 홈페이지 스타일]**
❌ "~이란?", "~의 정의", "~란 무엇인가"
❌ "주요 증상", "원인", "관리", "치료", "예방"
❌ 질환명으로 끝나는 소제목 (예: "오십견")
❌ 같은 핵심 단어 반복 사용 (3개 이상 소제목에 같은 단어)
❌ 질문형 소제목 ("~인가요?", "~일까요?")
❌ 설명형 소제목 ("~입니다", "~됩니다")

**[권장 - 생활 장면 중심 표현]**
✅ '이럴 때', '이런 밤에', '이 과정에서', '이후에 나타나는'
✅ '며칠째', '아침마다', '자다가', '옷 입을 때'
✅ 시간/상황 배경을 담은 표현
...
❌ "통증 완화 치료" → ✅ "옷 갈아입을 때마다 '아이고' 소리가 나올 때"
❌ "회복 과정" → ✅ "며칠이 지나도 나아지지 않는다면"

**[소제목 작성 원칙]**
1. 정보 정리가 아니라 '상황 설명'처럼 느껴질 것
2. 소제목 하나당 핵심 증상 단어 1회 이하
3. 소제목을 읽었을 때 "경험 이야기"처럼 느껴질 것
4. 칼럼·에세이 말투로 작성 (교과서/병원 안내문 ❌)
5. 5~7개 소제목 중 2개 이상은 시간/상황 배경 포함

**[소제목 자가 검증]**
□ "~이란?", "정의", "원인", "증상", "치료", "예방" 포함 안 됨?
□ 질환명으로 끝나지 않음?
□ 같은 단어가 3개 이상 소제목에 반복되지 않음?
□ 읽었을 때 의학 백과가 아니라 칼럼처럼 느껴지는가?
...
□ 소제목에 "원인/증상/치료/예방/관리/정의" 단어 없음 (블로그탭 필수!)
□ 소제목이 생활 장면 중심이며 의학 백과처럼 보이지 않음

**→ 10개 항목 전부 OK여야 합격! 하나라도 위반 시 전면 수정!**
...
🚨🚨🚨 **[절대 1순위] 작성 전 필독! 이것만은 절대 금지!!!** 🚨🚨🚨

**1. 모든 숫자 표현 절대 사용 금지!**
❌ "200여 종", "2~4회", "6~8회", "1~2주", "38도", "3일 이상", "30초"
❌ 모든 통계, 횟수, 기간, 온도, 개수 → 전부 금지!
✅ "매우 다양한", "드물지 않게", "자주", "며칠", "일정 기간"

**2. 상투적 마무리 표현 절대 금지!**
❌ "오늘 전해드린", "~되기를 바랍니다", "계기가 되기를", "도움이 되길"
✅ 핵심 요약이나 주의사항으로 자연스럽게 종료!

**3. 블로그 투 과다 사용 금지!**
❌ "한 번쯤 경험해보셨을 텐데요" - 상투적 도입부
❌ "~인데요", "~이죠", "~거든요" 과다 사용 (문장당 최대 1개!)
❌ "~하는 건 어떨까요?" - 블로그 질문 투
❌ "오늘은 ~에 대해 살펴보겠습니다" - 메타 설명 (즉시 본문으로!)

**4. 번역투 금지!**
❌ "~을 말하죠", "~을 의미하는 경우가 많습니다"
✅ "~입니다", "~을 가리킵니다"로 간결하게

위 4가지를 어기면 즉시 탈락! 작성 시작 전 3번 읽고 시작하세요!
...
🔥 **[최우선] AI 말투 완전 제거 - 자연스러운 글쓰기**
❌ **절대 금지:**
- AI 설명체, 교과서체, 논문체 → 실패!
- "~할 수 있습니다" 과다 사용 (문단당 최대 1회!)
- "~도움이 됩니다" 과다 사용 (글 전체 최대 2회!)
- "~언급됩니다", "~알려져 있습니다" 과다 사용 (글 전체 최대 2회!)
- 추상명사 반복 ("고려", "양상", "측면", "관점") → 최대한 배제!
- 같은 표현을 문단마다 반복 → 즉시 탈락!
- 광고 문장처럼 보이는 표현 → 실패!

🚨 **번역투 절대 금지 (한국어 글쓰기!):**
❌ **영어 → 한국어 번역투 표현 금지:**
1. 명사형 종결: "~하는 것이다", "~하는 것이 중요하다", "~하는 것입니다"
   - ✅ "~습니다", "~ㅂ니다", "~는 편입니다"로 간결하게
2. "~을 말하죠", "~을 의미하는 경우가 많습니다", "~을 가리킵니다"
   - ✅ "~입니다", "~를 뜻합니다"로 간결하게
3. "~에 있어서", "~에 대해서", "~에 관해서" 과다 사용
   - ✅ "~에서", "~때", "~면"으로 간결하게
4. "~함으로써", "~하기 위해서는", "~하는 것이 필요하다"
   - ✅ "~해서", "~하려면", "~하면 좋습니다"로 자연스럽게
5. 피동태 과다: "되어진다", "이루어진다", "여겨진다", "주어진다"
   - ✅ "됩니다", "이뤄집니다", "여겨집니다"로 짧게
6. 관계대명사식: "~하는 것", "~인 것", "~된 것"
   - ✅ 동사로 바로 표현
7. "~을 통해", "~로 인해", "~에 의해" 과다
   - ✅ "~로", "~해서", "~때문에"로 간결하게

❌ **번역투 예시:**
- "운동을 하는 것이 중요합니다" → ✅ "운동이 중요합니다"
- "염증을 일으키는 상태를 말하죠" → ✅ "염증이 생긴 상태입니다"
- "의학적으로 급성 비인두염을 의미하는 경우가 많습니다" → ✅ "의학적으로는 급성 비인두염입니다"
- "통증에 있어서 주의가 필요합니다" → ✅ "통증이 있을 때 주의가 필요합니다"
- "예방함으로써 건강을 지킬 수 있습니다" → ✅ "예방하면 건강을 지킬 수 있습니다"
- "치료가 되어지는 경우가 있습니다" → ✅ "치료되는 경우가 있습니다"
- "확인하는 것이 좋습니다" → ✅ "확인하면 좋습니다"

✅ **필수 작성법:**
- 실제 사람이 경험을 정리하듯 쓰되, 체험담처럼 보이지 않게
- 문장은 최대한 짧게 (한 문장에 정보 1개)
- 말로 설명하듯 자연스럽게 흐름 만들기
- '의학 정보 설명'보다는 '독자가 고개 끄덕이게 만드는 흐름' 우선
- 종결어미 다양화 필수: ~습니다, ~됩니다, ~ㅂ니다, ~는 편입니다, ~기도 합니다
- 번역투 없는 자연스러운 한국어!

✅ **완충 표현 다양화 (같은 표현 2회 이상 반복 금지!):**
- "~경우가 있습니다" → 문단당 최대 1회, 글 전체 최대 3회
- "~도움이 될 수 있습니다" → 글 전체 최대 2회
- 대체: "~는 편입니다", "~기도 합니다", "~로 알려져 있습니다", "~ㄹ 수 있습니다", "~나타날 수 있습니다"
...
2. **모든 의학 정보는 '조건·범위·차이'를 함께 제시**
   ...
   - 다양한 표현: "개인차가 있습니다", "상황에 따라 다릅니다", "사람마다 느끼는 정도가 다를 수 있습니다"
...
4. **'설명해주는 사람'의 톤 유지**
   ...
   - ✅ 설명형: "병원 방문이 필요한 경우입니다", "확인해보는 것이 좋습니다", "주의가 필요합니다"
...
[📝 문장 스타일 - 자연스러운 흐름]
- 한 문장에 정보 1개만! 길게 늘어지면 AI 냄새
- 짧은 문장으로 리듬감 있게 (20자~30자 내외)
- 설명 → 보충 → 완곡한 정리 구조
- ❌ 논문체·AI 요약체·교과서체 절대 금지!
- ✅ "~입니다" 연속 사용 금지 → "~습니다", "~됩니다", "~ㅂ니다", "~는 편", "~기도 합니다" 교차
- 독자가 고개 끄덕이게 만드는 흐름 → "아, 그래서 그렇구나" 느낌
- 광고 문장처럼 보이면 즉시 탈락!
...
7. 소제목은 질문형(H3)으로 작성 → "~이란?", "~증상은?" (검색 의도 정확 매칭)
   - 🎯 **소제목에서만 수사적 질문 활용 가능 (SEO 최적화)**
   - ✅ 허용: "무릎 통증, 왜 생기는 걸까요?", "이 증상, 언제 병원에 가야 할까요?"
   - ❌ 본문 내 반복적 질문 금지: "~인가요?" 연속 사용 시 AI 냄새
8. 🚨 **완충 표현 다양화 (같은 표현 반복 금지!):**
   - "~경우가 있습니다" → 문단당 최대 1회, 글 전체 최대 3회
   - "~도움이 될 수 있습니다" → 글 전체 최대 2회
   - "~알려져 있습니다" → 글 전체 최대 2회
   - ✅ 다양한 대체: "~는 편입니다", "~기도 합니다", "~나타날 수 있습니다", "~관찰됩니다"
9. 구체적 숫자/시간은 필요 시에만 사용 → 예방 실천법 등에서 "40~60분마다", "약 1분간" (치료·효과 수치는 절대 금지)
10. 🎯 **문장 리듬 예측 불가능하게 (AI 패턴 탈피!)**
    - 짧은 문장 (5~10자): "통증이 시작됩니다.", "붓기가 나타납니다."
    - 중간 문장 (15~25자): "무릎을 구부릴 때마다 뻐근한 느낌이 들기도 합니다."
    - 긴 문장 (30~40자): "계단을 내려갈 때 무릎 안쪽에서 걸리는 듯한 느낌과 함께 소리가 날 수 있습니다."
    - ❌ 균등한 길이 반복 → AI 냄새 즉시 발생!
    - ✅ 짧음→중간→짧음→긴 문장 순서를 예측 불가능하게 섞기
11. 📐 **문단 리듬 다양화 (네이버 최적화!)**
    - 각 소제목마다 1줄/3줄/2줄 문단 섞기 (균등하면 AI스럽게 보임!)
    - ✅ 네이버 블로그 최적 문단: 2~3줄 (모바일 가독성)
    - ❌ 5줄 이상 긴 문단 → 독자 이탈 증가
    - ❌ 1줄 문단 과다 → 성의 없어 보임
12. 🎭 **문장 종결 다양화 (AI 패턴 제거!)**
    - "~입니다" 연속 사용 금지 (최대 3문장 연속)
    - ✅ 교차 필수: "~습니다", "~됩니다", "~ㅂ니다", "~는 편입니다", "~기도 합니다", "~나타납니다"
    - ✅ 종결어미 변화 순서를 예측 불가능하게!

[💫 사람이 쓴 것처럼 - 자연스러운 글쓰기 추가 규칙]

13. 🎯 **감정 공감 포인트 (독자와 연결!)**
    - ✅ 허용 (의료법 안전): "이런 통증이 계속되면 일상생활이 불편할 수 있습니다", "밤에 잠을 설칠 정도라면 주의가 필요합니다"
    - ✅ 상황 공감: "아침에 일어날 때마다", "계단 내려갈 때", "오래 앉아있다가 일어설 때"
    - ❌ 금지 (체험담 느낌): "저도 이런 증상을 겪었는데요", "정말 힘드시죠", "너무 걱정하지 마세요"
    - ❌ 금지 (과도한 감정): "끔찍한 통증", "엄청난 고통", "심각한 문제"

14. 🏥 **구체적 상황 묘사 (리얼리티!)**
    - ✅ 아침 기상 시: "아침에 눈을 뜨고 일어나려는 순간", "자고 일어났을 때 뻣뻣한 느낌"
    - ✅ 일상 동작: "계단을 오르내릴 때", "쪼그려 앉았다 일어날 때", "오래 서있을 때"
    - ✅ 날씨 영향: "날씨가 흐린 날", "기온이 떨어지는 환절기", "비 오기 전"
    - ✅ 진행 과정: "처음에는 가볍게", "시간이 지나면서", "점차", "결국"
    - ❌ 금지 (체험담): "제 경우에는", "실제로 병원에 갔더니", "치료받고 나니"
    - ❌ 금지 (직접 권유): "당장 병원 가세요", "꼭 치료받으세요"

15. 😊 **이모지 전략적 사용 (네이버 SEO!)**
    - ✅ 소제목(H3)에만 사용! 본문 내 이모지 금지
    - ✅ 허용 이모지 (3~4종류만 선택):
      * 주의/경고: ⚠️, 🚨
      * 정보/포인트: 💡, 📌, 🎯, ※
      * 의료/건강: 🏥, 💊 (과도하게 사용 금지 - 광고 느낌)
      * 긍정/체크: ✅, 🔍
    - ❌ 금지 이모지: 😊😢😭❤️💕🎉🔥 (감정 이모지 - 광고 느낌)
    - ❌ 이모지 과다 사용 금지 (소제목당 최대 1개)

16. 🔍 **네이버 알고리즘 심화 (체류시간 + 재방문)**
    - ✅ 소제목에 호기심 유발: "무릎 통증, 이것만 확인하세요", "많은 분들이 놓치는 증상"
    - ✅ 검색 의도 매칭: "~이란?", "~원인은?", "~증상 체크", "~주의사항"
    - ✅ 정보 밀도 높이기: 각 문단마다 새로운 정보 1개 이상 포함
    - ✅ 체류 시간 증가 전략:
      * 문단 끝에 다음 내용 암시 (예: "하지만 더 중요한 건~", "특히 주의해야 할 경우는~")
      * 핵심 정보는 중간에 배치 (너무 빨리 주지 않기)
      * 문단 간 자연스러운 연결 (궁금증 유발)
    - ✅ 재방문 유도:
      * 계절별/상황별 정보 자연스럽게 언급 ("환절기에는~", "날씨가 추울 때~")
      * 후속 관심사 암시 ("이런 증상과 함께 ~도 확인해보는 것이~")
    - ❌ 제목 낚시 금지: 내용과 무관한 자극적 제목

17. 🎨 **구어체 절제 (과다 사용 = AI 냄새!)**

    🚨 **블로그 투 구어체 과다 사용 금지!**
    - ❌ "~인데요", "~이죠", "~거든요", "~잖아요" → 글 전체 합쳐서 최대 3~4개!
    - ❌ "한 번쯤 경험해보셨을 텐데요" - 상투적 블로그 도입부
    - ❌ "~하는 건 어떨까요?", "~해보시는 건 어떨까요?" - 블로그 질문 투
    - ❌ 문장마다 구어체 붙이면 AI 블로그처럼 보임!

    ✅ **적절한 구어체 사용법:**
    - "~입니다", "~습니다", "~됩니다" 를 기본으로 사용
    - "~인데요", "~이죠" 는 글 전체에서 2~3회만 사용 (강조할 때만!)
    - 친근한 표현: "많은 분들이", "이런 경우", "때로는"
    - 자연스러운 전환: "그런데", "하지만", "다만" (과다 사용 금지 - 5회 미만)

    ❌ 금지: 오타, 맞춤법 오류, 문법 오류, 지나친 구어체 ("ㅋㅋ", "ㅎㅎ", "~요~")

18. 👁️ **3인칭 관찰자 시점 강화 (체험담 아님!)**
    - ✅ 정확한 톤: "이런 증상이 나타나는 경우", "~를 경험하는 분들", "~로 불편을 느끼는 상황"
    - ✅ 객관적 설명: "일반적으로", "의학적으로는", "상황에 따라"
    - ❌ 절대 금지 (1인칭): "저는", "제가", "우리", "저희 병원"
    - ❌ 절대 금지 (2인칭 직접): "당신은", "여러분은" (독자 지칭은 "~는 분들"로)
    - ❌ 절대 금지 (체험담): "실제로 치료를 받은 사례", "환자분이 말씀하시길"

19. 📖 **문단 시작 패턴 다양화 (AI 패턴 탈피!)**
    - ✅ 설명형: "무릎 통증은 여러 원인이 있을 수 있습니다", "이런 증상이 나타나는 이유는~"
    - ✅ 상황형: "아침에 일어날 때", "계단을 내려갈 때", "오래 앉아있다가 일어서면"
    - ✅ 조건형: "만약 통증이 지속된다면", "이런 증상과 함께 붓기가 있다면"
    - ✅ 시간형: "처음에는 가볍게 시작되지만", "시간이 지나면서", "증상이 심해지면"
    - ✅ 비교형: "일반적인 통증과 달리", "다른 증상에 비해"
    - ❌ 같은 패턴 3회 연속 사용 금지 (예: "~는~", "~는~", "~는~" 반복)

[🧠 마케팅 최적화 - 자연스럽게]

20. 💝 **글 흐름: 공감 → 정보 → 신뢰 → 행동 (AIDA)**
    - 서론: 독자 상황 공감 ("아침에 일어날 때마다~")
    - 본론: 객관적 정보 + 의학적 근거 ("일반적으로", "연구에서는")
    - 결론: 자연스러운 행동 제시 ("한 번쯤 점검해보는 것도 좋습니다")
    - ✅ 사회적 증거: "많은 분들이", "흔히 나타나는"
    - ✅ 권위: "의학적으로", "전문가들이 주목하는"
    - ❌ 직접 권유 금지: "병원 가세요", "상담하세요"

[📝 추가 주의사항]
- 추상명사 배제: "고려/양상/측면" → "살펴보다/나타나다/느끼다"
- 같은 표현 반복 금지: "~경우가 있습니다" 3회 연속 사용 시 탈락

🚨 **마무리 상투적 표현 절대 금지 (블로그 AI 냄새!):**
❌ "오늘 전해드린", "오늘 소개해드린", "오늘 알아본"
❌ "~되기를 바랍니다", "~바라요", "~되셨으면 합니다"
❌ "도움이 되길 바랍니다", "도움이 되었으면 좋겠습니다"
❌ "점검해보는 계기가 되기를", "계기로 삼아보시길"
❌ "이 글이 ~하는 데 도움이 되었기를"

✅ **자연스러운 마무리 방법 (하나만 선택!):**
1. 핵심 내용 간단 요약으로 끝: "증상이 지속되거나 일상생활이 불편하다면 전문의 상담이 필요할 수 있습니다."
2. 주의사항 언급: "특히 갑작스러운 통증이나 붓기가 함께 나타난다면 빠른 확인이 필요합니다."
3. 그냥 본문 마지막 문장으로 자연스럽게 종료 (추가 마무리 멘트 없이!)

[HTML 구조]
⛔ 인사말 금지 ("안녕하세요") → ✅ 바로 일상 상황 묘사로 시작
<div class="naver-post-container">
  <p>도입: 독자 체험 묘사 (2~3문단)</p>
  <h3>소제목 (이모지 1개 + 질문형/정보형)</h3>
  <p>본문 내용 (문단 길이 다양하게: 1줄/3줄/2줄 섞기)</p>
  ${targetImageCount >= 1 ? '[IMG_1]\n' : ''}
  <h3>다음 소제목</h3>
  <p>본문 계속...</p>
  <p>마무리: 핵심 요약이나 주의사항으로 자연스럽게 종료 (상투적 멘트 금지!)</p>
  <p>#해시태그 10개</p>
</div>
...
[📤 최종 체크]
□ 의료법: 단정·지시·보장 표현 0개
□ AI 냄새: 숫자/통계/번호/추상명사 금지
□ 톤: "광고" 아닌 "설명" 느낌
□ 분량: ${targetLength + 100}~${targetLength + 200}자
  ...
  /* 기존 상세 프롬프트 주석 처리 (API 타임아웃 방지)
    ...
    ❌ **"~인지, ~인지, ~인지" 나열 패턴 절대 금지!**
       ...
       - **🚨 체크포인트 별도 섹션 절대 금지!:**
         ...
         ❌ "확인 사항" 소제목 후 불릿 리스트
         ❌ "점검 항목" 소제목 후 불릿 리스트
         → 모든 체크포인트는 반드시 문단 서술에 녹여서 자연스럽게 표현!
         → 별도 섹션으로 분리하면 AI smell + 의학백과 구조로 최하위 노출!
    ...
    📏 분량 전략:
    - 서론 10%, 본론 2~4개 섹션, 결론 10%, 해시태그 10개
    - 상한선 도달 시 즉시 마무리
    - 미달 시: 섹션 추가 또는 구체적 예시 추가
    
    이미지 개수: ${targetImageCount}장 ${targetImageCount > 0 ? `(${imageMarkers} 마커 사용)` : '(이미지 없이 텍스트만 작성)'}

    🚨 **이미지 마커 사용 주의사항:**
    - 이미지 마커([IMG_1], [IMG_2] 등)는 문단 사이에 삽입
    - ⚠️ 이미지 마커 뒤에도 반드시 다음 소제목과 본문 계속 작성!
    - 이미지 마커만 쓰고 멈추지 마세요!
    - 이미지는 단순히 시각 자료일 뿐, 글은 ${targetLength + 100}~${targetLength + 200}자까지 계속 작성

    [🚨 AI 냄새 제거 - 자연스러운 글쓰기 필수 규칙! 🚨]
    ⛔ **절대 금지 표현:**
    - 원 숫자 (①②③④⑤) 사용 금지
    - 연결어 과다 (그러나, 하지만, 그런데) → 5회 미만으로 제한
    - 강조 부사 남용 (매우, 굉장히, 상당히, 아주, 너무) → 6회 미만으로 제한

    🚨 **메타 설명 절대 금지 (AI 블로그 전형적 패턴!):**
    ❌ "오늘은 ~에 대해 살펴보겠습니다", "오늘은 ~에 대해 알아보겠습니다"
    ❌ "이번 글에서는 ~을 다뤄보겠습니다", "이 글에서는 ~에 초점을 맞췄습니다"
    ❌ "~에 대해 자세히 알아보겠습니다", "~에 대해 이야기해보겠습니다"
    ❌ 글쓴이의 의도나 목적을 설명하는 모든 문장 → 즉시 본문으로!

    - 교과서식 정의 (~란 무엇인가요?, ~이란 ~을 의미합니다) → 상황 묘사로 시작

    ✅ **자연스러운 표현 규칙:**
    - 모호한 강조 대신 구체적 묘사: "매우 많다" → "적지 않은 분들이", "상당히 높다" → "드물지 않게 나타나는"
    - 연결어 대신 문장 통합: "통증이 있습니다. 그러나 참을 만합니다" → "참을 만한 통증이 있습니다"
    - 실제 사례 묘사: "증상이 심합니다" → "밤잠을 설칠 정도로 아픕니다"
    - 자연스러운 시작: "요즘 아침에 일어나면 무릎이 뻐근하신 적 있으신가요?"
    - 종결어미 다양화: ~합니다(최대 4회), ~경우가 있습니다, ~편입니다, ~기도 합니다

    🚨🚨🚨 **본문에 모든 숫자 표현 절대 금지!!! (의료법 중대 위반!)** 🚨🚨🚨

    ⛔⛔⛔ **본문 HTML 작성 시 절대 사용 금지 (발견 즉시 탈락!):**

    1. ❌ **퍼센트/비율:** "90%", "80% 이상", "절반 이상", "70% 정도"

    2. ❌ **통계 표현:** "10명 중 7명", "2배 증가", "3배 많다", "200여 종"

    3. ❌ **횟수 표현 (가장 흔한 실수!):**
       - "2회에서 4회", "6회에서 8회", "년 2~4회", "하루 3번"
       - ✅ 대체: "드물지 않게", "자주", "여러 번", "때때로"

    4. ❌ **기간 표현 (절대 금지!):**
       - "1~2주", "3일에서 5일", "24시간", "2개월", "1년"
       - ✅ 대체: "며칠", "일정 기간", "시간이 지나면", "개인차가 있습니다"

    5. ❌ **순서 번호:** "첫째", "둘째", "셋째", "넷째"

    6. ❌ **숫자 리스트:** "1.", "2.", "3." (리스트 자체를 금지!)

    7. ❌ **개수/종류 표현:**
       - "5가지", "3개 방법", "7가지 증상", "200여 종", "10가지 이유"
       - ✅ 대체: "여러 가지", "다양한", "몇 가지", "여러"

    8. ❌ **구체적 수치:** "50mg", "10회", "3개월", "37.5도", "혈압 140"

    ⚠️ 위 규칙은 **본문 HTML 작성 시에만** 적용됩니다.
    (이 프롬프트 자체의 번호는 무시하세요 - 본문에만 적용!)

    ✅ **본문에서 반드시 사용할 대체 표현 (예시):**
    - ❌ "일 년에 2~4회 정도" → ✅ "드물지 않게 경험하는"
    - ❌ "6~8회 정도" → ✅ "자주 겪는 편"
    - ❌ "200여 종이 넘는다" → ✅ "매우 다양한 종류가 있습니다"
    - ❌ "1~2주 정도" → ✅ "며칠에서 일정 기간"
    - 횟수: "흔히 겪는", "자주 나타나는", "드물지 않은", "때때로"
    - 정도: "많은 분들이", "상당수가", "대부분의 경우", "일부에서"
    - 개수: "여러 가지", "다양한", "몇 가지"
    - 리스트는 자연스러운 문단으로 풀어쓰거나 HTML <ul><li>여러 가지</li><li>다양한 방법</li></ul> 형태로
    ...
    [📏 글자 수 체크 프로세스 - 단계별 확인!]
    🚨 **작성 중 글자 수 확인:**
    1. 서론 작성 후 → 누적 ${Math.round(targetLength * 0.1)}~${Math.round(targetLength * 0.15)}자 확인
    2. 본론 1 작성 후 → 누적 ${Math.round(targetLength * 0.3)}~${Math.round(targetLength * 0.35)}자 확인
    ...
    **1. conversion_score (0~100점)** - 의료법 준수하면서 전환력 측정
    공감 도입부(20) + 정보 가치(20) + 심리적 긴박감(20) + CTA 품질(20) + 전체 흐름(20)
    → "이거 내 얘기네!" 구체적 상황 묘사, 전문 인사이트, 간접적 행동 유도

    **2. safety_score (0~100점)** - 의료광고법 위반 감점
    직접 권유(-30), 효과 보장(-25), 과대 광고(-20), 비교 광고(-20), 공포 유발(-15), 병원 정보(-10)
    ✅ 안전: "검진을 고려해보는 것도 좋습니다", "전문의와 상담이 필요할 수 있습니다"
    ⚠️ conversion 100점이어도 의료법 위반 시 탈락!

    **3. ai_smell_score (0~100점)** - 낮을수록 좋음! 7점 이하 목표, 15점 초과 시 재작성
    문장 리듬 단조(25) + 판단 단정형(20) + 현장감 부재(20) + 템플릿 구조(15) + 가짜 공감(10) + 행동 유도 실패(10)
    → 종결어미/문장 시작 반복, "확인 필요" 남발, 상황 맥락 없음, 정의→원인→증상 순서
    판정: 0~7점(✅사람글) / 8~15점(⚠️경계) / 16점+(🚨AI확정)
    ...
    **🚨 블로그 이미지 텍스트 규칙 (매우 중요!):**
    ...
    □ 메타 설명 없음? ("오늘은 ~살펴보겠습니다", "~에 대해 알아보겠습니다" 금지!)
    □ 마무리에 상투적 표현 없음? ("오늘 전해드린", "되기를 바랍니다" 금지!)
...
4. 최신 정보를 적극 반영하되, 검증된 정보를 우선`;
    ...
    */
    ...
    // 🔧 fact_check 기본값 설정 (Gemini가 반환하지 않은 필드 보완)
    if (!result.fact_check) {
      result.fact_check = {};
    }
    // conversion_score가 없거나 0이면 기본값 70 설정 (콘텐츠가 생성되었으면 최소 70점)
