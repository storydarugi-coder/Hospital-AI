...
**금지 단어:** 가능성, 판단, 의심, 체크, 구분, 차이, 여부, 진단
**대체 표현:** ~한 경우가 있습니다, ~으로 언급되곤 합니다, ~한 상황이 있습니다
...
✅ 안전한 대체 표현:
"증상이 나타나기까지는 개인에 따라 하루 이상 시간이 걸리는 경우도 있으며, 경과 역시 사람마다 차이를 보일 수 있습니다."
"일부에서는 비교적 짧은 기간 내 증상이 가라앉기도 하지만, 상태에 따라 회복 속도는 달라질 수 있습니다."
...
→ '진료 현장' + '자주 본다' + '종종' = 간접적 경험·빈도 광고!
→ 의료광고 심의에서 자주 지적됨!

✅ 안전한 대체 표현:
"일반적으로 증상이 있을 때 수분 섭취가 부족해 상태가 악화되는 경우도 보고됩니다."
→ '우리 병원 경험' 제거, 일반 정보 출처로 전환!
...
1. **제목 구조 핵심 원칙 (🚨최우선!)**
   - 제목은 질병을 결론처럼 말하지 않고, 증상이 나타나는 상황이나 흐름에서 멈춘다
   - 병명으로 제목을 끝내지 않는다!
   - ❌ "~하면 OO일 수 있다" (병명이 결론)
   - ✅ "~한 증상이 반복될 때" (흐름에서 멈춤)

2. **병원 안내문 느낌 단어 금지**
   ❌ 관리, 이해, 특징, 연관, 원인, 증상, 치료, 예방, 진단
   → 이런 단어는 병원 홈페이지/안내문처럼 보임!
   ✅ 대체어: 흐름, 양상, 신호, 상황, 때, 경우

3. **병명 사용 원칙 (보조 역할만!)**
   - 병명은 제목 전체에서 보조 설명 역할만 하도록 제한
   - 병명이 제목의 주인공이 되면 안 됨!
   - ❌ "갑상선기능저하증 증상, 추위 타는 이유?" → 병명이 주인공
   - ❌ "갑상선기능저하증의 특징과 관리" → 병원 안내문
   - ✅ "유난히 추위를 많이 탄다면, 갑상선과 관련된 흐름일 수도" → 증상이 주인공, 병명은 보조

4. **안전한 제목 구조 패턴**
   ❌ "~일 수 있을까요?" → ✅ "~한 경우 자주 언급되는 상황"
   ❌ "~판단 기준" → ✅ "~살펴볼 신호"
   ❌ "~구분법" → ✅ "~에서 자주 나타나는 양상"
   ❌ "~체크 포인트" → ✅ "~에서 주목하는 부분"
   ❌ "OO의 원인과 증상" → ✅ "이런 증상이 반복될 때"

5. **계절·상황·행동 맥락 포함**
   ✅ 겨울철, 아침, 반복되는, 씻고 나서, 밤마다, 식후에 등
   ✅ 감정 어휘: 답답한, 찝찝한, 불편한 (허용)

6. **SEO 최적화**
   - 메인 키워드는 제목 앞부분에 배치 (네이버 SEO 기준)
   - 제목 길이: ${lengthGuide}
...
**❌ AI 같은 종결어 (피해야 할 단어)**
- 흐름, 상황, 시점, 사례, 과정
→ 이런 단어로 끝나면 "조건 충족형 문장"으로 보임!

**✅ 사람 같은 종결어 (대체 단어)**
- 경우, 순간, 뒤, 때, 이후, 느낌

**예시:**
❌ "스키장 부상 시 들린 무릎 똑 소리, 붓기가 잘 빠지지 않는 흐름"
✅ "스키장에서 다친 뒤 무릎에서 똑 소리가 나고 붓기가 오래 가는 경우"

❌ "전방십자인대 파열 관련 내용이 다루어지는, 무릎 비틀림 상황"
✅ "무릎이 비틀린 뒤 전방십자인대 손상이 이야기되는 상황"

❌ "겨울철 운동 중 무릎이 꺾이며 다리에 힘이 들어가지 않는 시점"
✅ "겨울철 운동 중 무릎이 꺾이고 다리에 힘이 빠지는 순간"
...
- 각 제목은 서로 다른 심리 트리거 사용:
  1. 호기심형 (궁금증 유발 - 단, 질문형 금지!)
  2. 공감형 (독자 상황 공감)
  3. 정보전달형 (유용한 정보 제공 느낌)
  4. 시기고정형 (특정 시점/상황)
  5. 맥락설명형 (상황과 연관성 설명)

- SEO 점수: 70~95점 사이로 현실적으로 평가
- type: 위 5가지 트리거 중 하나 (호기심/공감/정보전달/시기고정/맥락설명)

[검수 기준]
- 병·의원 블로그에 게시해도 장기간 노출 가능해야 함
- 의료 광고로 오해받을 표현이 없어야 함
- 독자가 '정보 글'로 인식할 수 있어야 함
- 🤖 AI 냄새 체크: "흐름/상황/시점/사례/과정"으로 끝나면 탈락!`;
