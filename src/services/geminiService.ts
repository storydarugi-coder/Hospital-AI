...
- description: ❌ 표지에서는 설명 최소화! 한 줄만 또는 생략
- 💡 표지는 "왜 봐야 하는지"만 전달
...
**${slideCount}장 - CTA (명령형 금지!)**
- subtitle: 4~8자 (예: "미루지 않는 습관")
- mainTitle: ❌ "~하세요" 금지! ✅ "~시점입니다" (예: "이 단계에서는\\n지켜보기보다 확인이 먼저입니다")
- description: 간접 표현만 (예: "반복된다면 전문적인 판단이 필요할 수 있습니다")
...
[🎯 최종 체크리스트]
1. 각 카드 description이 2줄 이상인가? → 1줄로 줄여라!
2. "~하세요" 명령형이 있는가? → "~시점입니다", "~단계입니다"로 바꿔라!
3. 설명이 판단보다 많은가? → 판단만 남겨라!
4. 표지에 긴 설명이 있는가? → 삭제하거나 최소화!
5. CTA에 시술명(스킨부스터 등)이 있는가? → "관리 방향", "관리 기준"으로 대체!
6. "맞춤형", "개인맞춤" 표현이 있는가? → "상태에 맞는"으로 대체!
