...
`;
        ...
        // 영감 모드: 느낌만 참고하여 재창작 (더 강력한 프롬프트)
        refImagePrompt = `
🎨🎨🎨 [스타일 재가공 모드 - 중요!] 🎨🎨🎨
첨부된 참고 이미지를 분석하고, 그 **스타일과 느낌을 살려서** 새로운 카드뉴스를 만드세요!

🔍 참고 이미지에서 분석할 것:
1. **배경 스타일**: 단색? 그라데이션? 패턴? → 비슷한 느낌으로 적용
2. **색상 팔레트**: 주요 색상 2~3가지 추출 → 유사한 색조 사용
3. **폰트 스타일**: 굵은 고딕? 둥근 폰트? 손글씨? → 비슷한 무게감/스타일
4. **일러스트 스타일**: 3D? 2D? 플랫? 귀여운? 전문적? → 동일한 스타일 적용
5. **전체 레이아웃 구조**: 텍스트 위치(상단/중앙/하단), 여백 비율 → 비슷한 구도
6. **디자인 요소**: 테두리, 그림자, 아이콘 스타일 → 유사하게 적용

⚠️ 핵심 지시사항:
- 참고 이미지의 **디자인 언어**를 학습하고 적용하세요
- 색상은 정확히 같을 필요 없지만, **톤앤매너**는 유지하세요
- 레이아웃 구조는 참고하되, 내용에 맞게 **유연하게 조정**하세요
- 참고 이미지가 브라우저 창 스타일이면 비슷한 프레임 적용
- 참고 이미지가 미니멀하면 미니멀하게, 화려하면 화려하게

❌ 하지 말 것:
- 참고 이미지를 무시하고 완전히 다른 스타일로 만들기
- 참고 이미지의 텍스트를 그대로 복사하기

✅ 최종 목표: 참고 이미지와 **같은 디자이너가 만든 것처럼** 보이는 새 카드뉴스!
`;
      }
    }
...
**${slideCount}장 - 마지막 표지/CTA (명령형 금지!)**
- subtitle: 4~8자 (예: "미루지 않는 습관")
- mainTitle: ❌ "~하세요" 금지! ✅ "~시점입니다" (예: "이 단계에서는\\n지켜보기보다 확인이 먼저입니다")
...
3. 각 카드 description이 2줄 이상인가? → 1줄로 줄여라!
4. "~하세요" 명령형이 있는가? → "~시점입니다", "~단계입니다"로 바꿔라!
5. 설명이 판단보다 많은가? → 판단만 남겨라!
6. CTA에 시술명(스킨부스터 등)이 있는가? → "관리 방향", "관리 기준"으로 대체!
7. "맞춤형", "개인맞춤" 표현이 있는가? → "상태에 맞는"으로 대체!

[💡 CTA 카드 모범 답안 (반드시 이 패턴으로!)]
✅ mainTitle: "이 단계에서는\\n관리 방향을 다시 잡아야 합니다"
✅ description: "반복되는 불편함은 상태에 맞는 관리 기준이 필요할 수 있습니다."
→ 시술명 ❌ / "방향"과 "기준" ✅`;
