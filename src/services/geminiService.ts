...
❌ 과한 경고 ("위험!", "긴급!", "지금 당장!")
❌ "돌연사", "사망", "좽음" → 공포 유발 키워드 금지! (→ "혁관 건강 위험 신호" 대체)
...
` : '';
    ...
    if (referenceImage) {
      if (copyMode) {
        ...
        refImagePrompt = `
🚨🚨🚨 [프레임 복제 모드] 🚨🚨🚨
제공된 이미지는 '내용 참고'가 아니라 '카드 프레임·레이아웃·타이포그래피 스타일 참고용'입니다.
다음 요소들만 그대로 유지하고, 텍스트 내용·주제·메시지는 전부 새로 작성하세요.

[✅ 유지해야 할 요소]
- 카드 전체 외곽 형태(모서리 라운드, 테두리 두께, 그림자 유무)
- 내부 구조(상단/중앙/하단 영역 분리 방식)
- 여백 비율과 정렬 방식(중앙 정렬, 위계 구조)
- 글씨체의 성격
  · 제목: 굵고 직선적인지 / 둥근지
  · 부제: 가벼운지 / 보조적인지
  · 강조 문구: 색상·크기 차이로 강조되는 방식
- 전체적인 UI 느낌(웹 카드, 카드뉴스, 앱 화면 등)
- 브라우저 창 버튼(빨강/노랑/초록)이 있다면 반드시 포함!

[❌ 변경해야 할 요소]
- 모든 텍스트 내용 → 내가 제공하는 내용으로!
- 일러스트·아이콘의 의미(단, 위치·크기·역할은 유지)

⚠️ '같은 디자인인데 다른 내용'처럼 느껴져야 함!
`;
      } else {
        // ========== 재가공 모드 ==========
        refImagePrompt = `
🎨🎨🎨 [디자인 프레임 재가공 모드] 🎨🎨🎨
제공된 이미지는 '디자인 프레임 참고용'입니다.
카드 테두리, 내부 구조, 여백, 타이포그래피 분위기만 유지하고
이미지 안의 모든 내용은 절대 그대로 사용하지 말고 완전히 새로 재가공하세요.

[🚫 절대 금지]
- 원본 이미지의 문구, 숫자, 표현, 주제, 메시지 재사용
- 의미·문맥·의도까지 유사하게 따라 하는 것
- 원본을 떠올릴 수 있는 키워드나 상황 설정

[✅ 반드시 유지]
- 카드 외곽 형태(프레임, 모서리 라운드, 그림자)
- 레이아웃 구조(상/중/하 또는 좌/우 분할 방식)
- 정보 위계(소제목 → 메인 강조 → 보조/설명)
- 글씨체의 성격(굵기 대비, 직선/곡선 인상, 강조 방식)
- 전체 UI 분위기(웹 카드 / 카드뉴스 / 앱 화면 느낌)

[📝 재가공 원칙]
- 내가 제공하는 주제·메시지로 완전히 새로 작성
- 문구는 원본과 단어·리듬·길이가 겹치지 않게 작성
- 같은 디자인 틀에 '다른 콘텐츠를 얹은 느낌'이 목표
- 한국어 기준 가독성 높은 정보형 문장 사용

⚠️ 원본 콘텐츠와의 연상 가능성을 의도적으로 차단하세요!
`;
      }
    }
...
4️⃣ 질환명은 단정하지 말고 가능성 구조 사용
   ✅ "~증상일 수 있는"
   ✅ "~의심될 수 있는"
   ✅ "~구분이 필요한"
   ❌ "당신은 ~입니다" (단정 금지)

5️⃣ 계절·상황·행동 맥락을 포함해 검색 의도 정확히 겨냥
   ✅ 겨울철, 아침, 반복되는, 씻고 나서, 밤마다, 식후에 등

6️⃣ 메인 키워드는 제목 앞부분에 배치 (네이버 SEO 기준)
   ✅ "피부건조 원인, 겨울철에 심해지는 이유"
   ❌ "겨울철에 심해지는 피부건조 원인"

7️⃣ 제목 길이: 28~38자 이내 (모바일 최적화)

8️⃣ 감정 어휘는 사용하되 자극적으로 쓰지 않는다
   ✅ 답답한, 찝찝한, 불편한, 반복되는 (허용)
   ❌ 무섭다, 위험하다, 심각하다 (금지)
...
[🎯 커스텀 스타일] "${customImagePrompt}" - 모든 카드에 적용 필수!
` : '';
...
- 2번~: 상단(부제) → 중상단(제목) → 중앙(일러스트) → 하단(설명, 있는 경우만)
${hasWindowButtons ? '- 브라우저 창 버튼(빨/노/초) 포함' : ''}

[필수 규칙]
- 1:1 정사각형 카드뉴스, 배경색 ${bgColor}
- 텍스트 잘림 없음 (여백 10% 이상)
- 해시태그 금지, 한국어로만 작성
...
- description: 판단 1줄만! (15~25자)
  ...
  ❌ 2~3문장 설명 금지!
- imageKeyword: 영어 3~4개 (예: "winter, blood vessel, health check")
...
JSON 형식:
{
  ...
  "imageKeyword": "english, keywords, here"
}
`;
