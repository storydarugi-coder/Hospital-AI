...
20. 🎯 **심리학적 설득 기법 (의료법 안전 범위 내)**
    - ✅ 손실회피 심리 (자연스럽게):
      * ❌ "놓치면 후회합니다" (직접적, 공포 조장)
      * ✅ "미리 알아두면 대처하기 수월할 수 있습니다" (정보 가치 강조)
      * ✅ "단순히 넘어가기엔 아쉬운 부분이 있습니다" (호기심 유발)
    - ✅ 사회적 증거 (자연스러운 표현):
      * ✅ "많은 분들이 경험하는 증상 중 하나입니다"
      * ✅ "이런 경우를 접하는 경우가 적지 않습니다"
      * ✅ "비슷한 상황에서 확인해보는 분들이 있습니다"
      * ✅ "흔히 나타나는 증상으로 알려져 있습니다"
    - ✅ 권위/전문성 (은근하게):
      * ✅ "의학적으로 살펴보면", "임상에서는", "연구에 따르면"
      * ✅ "전문가들이 주목하는 부분은", "의료진이 확인하는 과정은"
      * ❌ "저희 병원은", "우리 원장님은" (1인칭 금지)
    - ✅ 희소성 (자연스럽게):
      * ✅ "이 시기에 특히 주의가 필요한 부분입니다"
      * ✅ "간과하기 쉬운 신호 중 하나입니다"
      * ❌ "지금만", "한정" (직접적 긴급성 금지)

21. 📈 **전환 최적화 - 스토리텔링 구조 (CTA without 직접 권유)**
    - ✅ AIDA 프레임워크 자연스럽게 적용:
      * Attention (서론): 공감 가능한 상황 묘사로 시작
      * Interest (본론 초반): 독자가 몰랐던 정보 제공
      * Desire (본론 중반): 해결 가능성 암시 (직접 광고 아님)
      * Action (결론): 자연스러운 다음 단계 제시 (직접 권유 아님)
    - ✅ PAS 공식 (Problem-Agitate-Solve):
      * Problem: 독자의 문제 상황 공감 (1~2문단)
      * Agitate: 문제의 영향 구체화 (과장 없이)
      * Solve: 정보 제공 + 확인 방법 안내 (광고 아님)
    - ✅ 스토리 호 (Story Arc):
      * 도입: 일상적 상황에서 시작
      * 전개: 증상/문제 구체화
      * 위기: 주의가 필요한 시점 제시
      * 해결: 확인 방법/관리 방법 정보 제공
      * 결말: 독자 스스로 판단할 수 있는 기준 제공

22. 💝 **감정 공감 → 정보 → 신뢰 → 행동 동선**
    - ✅ 1단계: 감정 공감 (서론 1~2문단)
      * "아침에 일어날 때마다 무릎이 뻐근하다면"
      * "계단을 내려갈 때 불안한 느낌이 든다면"
      * 독자의 현재 상태에 공감하는 문장으로 시작
    - ✅ 2단계: 객관적 정보 제공 (본론)
      * 감정에서 벗어나 객관적 의학 정보로 전환
      * "이런 증상은 ~로 인해 나타날 수 있습니다"
      * 원인, 과정, 특징을 차분하게 설명
    - ✅ 3단계: 신뢰 구축 (본론 후반)
      * 정확한 정보 + 근거 제시 (출처 없이 자연스럽게)
      * "일반적으로", "의학적으로", "연구에서는"
      * 여러 가능성 제시 (단정 피하기)
    - ✅ 4단계: 자연스러운 행동 유도 (결론)
      * ❌ "지금 바로 병원 방문하세요" (직접 권유)
      * ✅ "이런 증상이 반복된다면 정확한 확인이 필요할 수 있습니다"
      * ✅ "단순히 넘기기보다는 한 번쯤 점검해보는 것도 방법입니다"
      * 독자 스스로 결정할 수 있는 여지 제공

[📝 마무리 문단 - 행동의 씨앗 심기]
23. 마지막 문단은 "좋은 말"로 끝내지 말고 행동 유도:
    ❌ "유용한 기준이 될 수 있기를 바랍니다" (아무 행동 안 남김, 추상적)
    ❌ "도움이 되었으면 좋겠습니다" (광고 문장 같음)
    ✅ "반복되는 증상이 있다면 단순히 넘기기보다는 자신의 몸 상태를 한 번쯤 점검해보는 계기로 삼아보는 것도 좋습니다."

13. 🚨 **추상명사 최대한 배제:**
    ❌ "고려", "양상", "측면", "관점", "기준", "판단", "도움"
    ✅ 구체적 동작/상황으로 대체: "살펴보다", "확인하다", "나타나다", "느끼다"

14. 🚨 **같은 표현 문단 반복 절대 금지:**
    - "~경우가 있습니다" 연속 3문단 사용 → 즉시 탈락!
    - 각 문단마다 다른 종결어미 사용 필수
    - 문단 시작 패턴도 다양화 (질문형, 상황 묘사형, 설명형 교차)
...
반드시 JSON 형식으로 응답하고, 목표 길이(공백 제외 ${targetLength + 100}~${targetLength + 200}자)를 준수해주세요.
  ...
  /* 기존 상세 프롬프트 주석 처리 (API 타임아웃 방지)
    ...
    - 결론: 100~150자
    - 해시태그: 10개
    - ⚠️ 2200자 절대 넘기지 마세요!` : targetLength >= 1500 ? `
    - 🟢 1500자 = 짧고 간결한 글 (딱 이 분량만!)
    - 서론: 100~150자
    - 본론 섹션: 2개 섹션 (각 섹션 250~300자)
    - 결론: 80~100자
    - 해시태그: 8~10개
    - ⚠️ 1600자 절대 넘기지 마세요!` : `
    - 🟢 적당한 길이
    ...
    🚨🚨🚨 **본문에 숫자/퍼센트/통계 절대 금지!!! (의료법 중대 위반!)** 🚨🚨🚨

    ⛔⛔⛔ **본문 작성 시 절대 사용 금지 (즉시 탈락!):**
    - ❌ 퍼센트/비율: "90%", "80% 이상", "절반 이상"
    - ❌ 통계 표현: "10명 중 7명", "2배 증가", "3배 많다"
    - ❌ 순서 번호: "첫째", "둘째", "셋째", "넷째"
    - ❌ 숫자 리스트: "1.", "2.", "3." (리스트 금지!)
    - ❌ 개수 표현: "5가지", "3개 방법", "7가지 증상"
    - ❌ 구체적 수치: "50mg", "10회", "3개월"
    ...
    ✅ **본문에서 사용할 대체 표현:**
    - "흔히 겪는", "자주 나타나는", "드물지 않은", "적지 않은"
    - "많은 분들이", "상당수가", "대부분의 경우", "일부에서"
