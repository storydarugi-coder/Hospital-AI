...
⚠️ 목표 길이: 공백 제외 ${targetLength}자 반드시 맞추기! (최대 ${Math.floor(targetLength * 1.05)}자까지만!)
🚨 예시: 1600자 목표 → 1680자 넘으면 안 됨! (2700자는 절대 금지!)
...
🚨 중요: ${Math.floor(targetLength * 1.05)}자 넘으면 안 됨! (예: 1600자 목표 → 1680자 초과 금지!)
  `;
  ...
  /* 기존 상세 프롬프트 주석 처리 (API 타임아웃 방지)
    ...
    ✅ **허용 범위:** ${targetLength}자 ~ ${Math.floor(targetLength * 1.05)}자 (상한선 엄격!)
    🎯 **이상적:** ${targetLength}자 ~ ${Math.floor(targetLength * 1.02)}자 (목표치에 최대한 근접!)
    
    📌 **분량 제한 규칙 (공백 제외):**
    - 1500자 설정 → 1500~1575자 범위 (1575자 넘으면 즉시 중단!)
    - 1600자 설정 → 1600~1680자 범위 (1680자 넘으면 즉시 중단!)
    - 2000자 설정 → 2000~2100자 범위 (2100자 넘으면 즉시 중단!)
    - ⚠️ 상한선 5% 초과 시 즉시 중단하고 마무리!
    - 🚨 예: 1600자 목표면 1680자 초과 절대 금지! (2700자는 너무 많음!)
    ...
    - 결론: 300자 (핵심 요약 + CTA)
    - 해시태그: 10~15개
    - ⚠️ 최대 5250자까지만!` : targetLength >= 4000 ? `
    - 🔴 4000자 = 긴 글!
    - 서론: 350자
    ...
    - 결론: 250자
    - 해시태그: 10개
    - ⚠️ 최대 4200자까지만!` : targetLength >= 3000 ? `
    - 🟡 3000자 = 중간 길이 글
    - 서론: 300자
    ...
    - 결론: 200자
    - 해시태그: 10개
    - ⚠️ 최대 3150자까지만!` : targetLength >= 2000 ? `
    - 🟢 2000자 = 적당한 길이 (딱 이 분량만!)
    - 서론: 150~180자
    - 본론 섹션: 2~3개 섹션 (각 섹션 300~320자)
    - 결론: 100~130자
    - 해시태그: 10개
    - ⚠️ 최대 2100자까지만! (2100자 넘으면 실패!)` : targetLength >= 1500 ? `
    - 🟢 1500자 = 짧고 간결한 글 (딱 이 분량만!)
    - 서론: 100~120자
    - 본론 섹션: 2개 섹션 (각 섹션 250~270자)
    - 결론: 80~100자
    - 해시태그: 8~10개
    - ⚠️ 최대 1575자까지만! (1575자 넘으면 실패!)` : `
    - 🟢 적당한 길이
    - 서론: 간결하게
    - 본론: 2~3개 섹션
    - 결론: 짧게
    ...
    - **${Math.floor(targetLength * 1.05)}자보다 많이 쓰는 것 (5% 초과! 즉시 중단!)**
    - 🚨 예시: 1600자 목표 → 1680자 넘으면 안 됨! (2700자는 절대 금지!)
    - 내용 없이 같은 말 반복하기
    - 해시태그로 글자 수 채우기
    - 불필요하게 길게 늘려쓰기
    ...
    🚨 **분량 초과 방지 (매우 중요!):**
    - 최대 ${Math.floor(targetLength * 1.05)}자까지만! (상한선 엄격 준수)
