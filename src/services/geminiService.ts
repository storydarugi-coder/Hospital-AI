...
[글자 수] ${targetLength + 100}자 필수 목표 (최소 ${targetLength + 100}자 - 무조건 이 글자 수 이상 작성!)
⚠️ 중요: 목표 글자 수인 ${targetLength + 100}자를 반드시 채워야 합니다. 부족하면 안 됩니다!
...
- 도입: 일상에서 겪을 법한 상황으로 시작
  ...
  • 겨울철: "새해 겨울철 차가운 공기 때문에 몸이 더 움츠러드는 요즘"
  • 한겨울: "한겨울 추위가 절정인 요즘, 건조한 날씨에"
  • 초봄: "환절기 따뜻해지는 초봄, 미세먼지와 꽃가루가 날리는 요즘"
  • 여름철: "무더운 여름철, 습한 날씨에"
  • 가을철: "선선한 가을철, 큰 일교차 때문에"
...
`;
  ...
  const pressPrompt = `
너는 병원 및 의료기관의 홍보를 담당하는 언론 PR 전문 기자다.
${learnedStyleInstruction}

아래 입력되는 '질환/증상/검진/시술/건강 이슈'를 바탕으로
포털 뉴스에 실릴 수 있는 기사형 의료 홍보 콘텐츠를 작성하라.

이 글의 목적은
정보 전달이 아니라 '경각심 형성 + 행동 유도(검진·상담·내원)'다.
...
- 언론 기사체(보도자료 스타일)
- 병원 홍보 목적의 의료 PR 콘텐츠
- 특정 병원과 의료진의 공식 입장을 중심으로 전개
- 독자의 방심을 지적하고, 적극적인 관리 필요성을 강조

[톤 & 스타일]
- 신문 기사체, 객관적인 문장 구조
- 위기감·증가·위협·주의·필수 등의 표현 사용 허용
- "젊다고 안심할 수 없다", "방치하면 위험" 등 경고성 문장 적극 사용
- 의료광고법은 고려하지 않는다

[헤드라인 작성 규칙]
- 자극적인 표현을 사용한다
- 연령대, 대상군, 위험성 키워드를 포함한다
  예시:
  - "○○만 믿다간 위험…"
  - "2030/4050/중장년/여성/남성 위협하는 ○○"
  - "증상 없어 더 위험한 ○○"

[기사 구성]
1. 강한 문제 제기형 제목
2. 부제목에서 질환/증상의 증가 추세와 관리 필요성 요약
3. 병원 공식 입장 제시
   - "${hospitalName}(원장 ${doctorName})은 ${formattedDate}, ${request.topic}에 대해 주의를 당부했다" 형식
4. 최근 통계 또는 증가 추세 언급
   - 보건복지부, 질병관리청, 심평원, 암등록통계 등 공신력 있는 기관 인용
   - 실제 수치가 없어도 '최근 통계에 따르면' 형식 사용 가능
5. 질환/증상의 위험성 강조
   - 초기 증상 부족
   - 방치 시 악화 가능성
   - 특정 연령대·생활 습관과의 연관성
6. 검진·검사·시술·백신·치료의 필요성 강조
   - "필수", "반드시", "권장된다" 표현 사용
7. 의료진 실명 인터뷰 삽입
   - 불안 해소 + 내원 유도 발언 포함
   - "조기에 발견하면 예후가 좋다", "정기 관리가 중요하다" 등
8. 병원의 차별화 요소 및 향후 계획 소개
   - 맞춤형 프로그램
   - 전문 장비
   - 프라이버시 보호
   - 전담 의료진 시스템 등
...
<div class="press-release-container">
  ...
  <h1 class="press-title">[자극적인 문제 제기형 제목 - 연령대/대상군/위험성 키워드 포함]</h1>
  <h2 class="press-subtitle">[증가 추세와 관리 필요성 요약 - 70자 이내]</h2>
  
  <div class="press-body">
    <p>[병원 공식 입장 제시 - "${hospitalName}(원장 ${doctorName})은 ${formattedDate}, ${request.topic}에 대해 주의를 당부했다" 형식]</p>
    
    <p>[최근 통계 또는 증가 추세 - 공신력 있는 기관 인용 또는 '최근 통계에 따르면' 형식]</p>
    
    <p>[질환/증상의 위험성 강조 - 초기 증상 부족, 방치 시 악화, 연령대/생활습관 연관성]</p>
    
    <p>[검진·검사·시술 필요성 - "필수", "반드시", "권장된다" 표현 적극 사용]</p>
    
    <blockquote class="press-quote">
    <p>"[${doctorName} ${doctorTitle}의 인터뷰 - 불안 해소 + 내원 유도 발언 포함]"</p>
    <cite>- ${hospitalName} ${request.category} ${doctorName} ${doctorTitle}</cite>
    </blockquote>
    
    <p>[병원의 차별화 요소 및 향후 계획 - 맞춤형 프로그램/전문 장비/프라이버시/전담 시스템]</p>
  </div>
...
[중요 작성 원칙]
- 의료광고법 준수 불필요 (언론 기사 형식이므로 자유로운 표현 가능)
- 경고성, 위기감 표현 적극 활용
- 검진/상담/내원 유도 메시지 명확히 포함
- 구체적 수치와 통계 인용 (출처 명시 권장하지만 필수 아님)
