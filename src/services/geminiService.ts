...
5. **텍스트 절대 금지**: 이미지 안에 글자/문자/텍스트 절대 넣지 마세요! (숫자도 금지)
6. 로고는 절대 포함하지 말 것
...
export const generateSingleImage = async (promptText: string, style: ImageStyle = 'illustration', aspectRatio: string = "16:9"): Promise<string> => {
    ...
    // 전체 한국어 프롬프트 (텍스트 거의 금지)
    const finalPrompt = `${stylePrompt}. ${promptText}. 중요: 이미지 안에 텍스트/글자/문자 절대 넣지 마세요. 로고 금지, 무서운 요소 금지, 전문적인 한국 의료 맥락.`;
...
export const generateBlogPostText = async (request: GenerationRequest): Promise<{ 
  ...
  const blogPrompt = `
    ...
    **🚫 텍스트 절대 금지 (매우 중요!):**
    - 이미지 안에 글자/문자/텍스트 절대 넣지 마세요!
    - 숫자도 넣지 마세요!
    - 로고, 워터마크, 라벨 금지!
    - 오직 시각적 요소(인물, 배경, 장기, 의료기기 등)만 포함!
    ...
    ${imageStyle === 'illustration' 
      ? '- "밝은 병원 진료실에서 의사가 환자에게 설명하는 모습, 3D 일러스트, 아이소메트릭 뷰, 클레이 렌더, 파란색 흰색 팔레트, 텍스트 없음"'
      : imageStyle === 'medical'
      ? '- "인체 심장의 3D 단면도, 좌심실과 우심실이 보이는 해부학적 구조, 혈관과 판막이 보이는 의학 일러스트, 파란색 배경, 텍스트 없음"'
      : '- "깔끔한 병원 진료실에서 의사가 환자와 상담하는 모습, 실사 사진, DSLR 촬영, 자연스러운 조명, 텍스트 없음"'}
  `;
  ...
  const cardNewsPrompt = `
    ...
    **🚫 텍스트 절대 금지 (매우 중요!):**
    ...
    예시 (${imageStyle === 'illustration' ? '3D 일러스트' : imageStyle === 'medical' ? '의학 3D' : '실사 사진'} 스타일):
    ${imageStyle === 'illustration' 
      ? '- "밝은 병원 배경의 건강 인포그래픽, 3D 일러스트, 아이소메트릭 뷰, 클레이 렌더, 파란색 흰색 팔레트, 텍스트 없음"'
      : imageStyle === 'medical'
      ? '- "인체 폐의 3D 단면도, 기관지와 폐포 구조가 보이는 해부학 일러스트, 투명 효과, 파란색 의료 배경, 텍스트 없음"'
      : '- "깔끔한 병원 환경 이미지, 실사 사진, DSLR 촬영, 전문적인 분위기, 텍스트 없음"'}
  `;
