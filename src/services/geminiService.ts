...
**4. 구조: Gemini 교과서식 탈피 → GPT 관찰-해석-정리 구조**
   ...
   **도입부 필수:**
   - 바로 생활 장면으로 시작 (정의 금지)
   - 예: "아침에 일어나서 첫 발을 디딜 때 발뒤꿈치가 아프다면...", "목이 불편해서 고개 돌리기 힘들 때..."
...
**✅ 허용되는 완화 표현 (의료진 직접 언급 최소화!):**
- "확인이 필요할 수 있다" (❌ "전문의 상담" 대신)
- "진료를 통해 점검" (❌ "병원 방문" 대신)
- "가까운 곳에서 체크" (중립적 표현)
- "객관적 기준으로 확인"
- 🚨 "의사", "전문의", "전문가" 직접 언급 최대한 자제!

[중요]
[📝 제목 생성 강화 - 검색형 제목 필수!]
[중요]

**🎯 제목은 실제 검색어처럼 작성! 네이버 검색창에 입력하는 형태!**

**✅ 검색형 제목 공식 (사람들이 실제로 검색하는 형태):**
[시기/상황] + [구체적 증상 조합] + [궁금증/의문형] + [🚨 숫자 없이!]

**핵심 원칙:**
- 사람들이 네이버에 직접 타이핑하는 검색어 형태
- "어떻게", "왜", "~인지", "~일까", "~할까" 같은 궁금증 표현
- 증상을 조합해서 표현 ("A인데 B는 없고", "A 하면서 B도")
- 🚨 숫자 완전 금지! ("3일", "2주", "50대", "90%" 등)

**예시:**
✅ "요즘 코막힘인데 콧물은 안 나와요 - 감기 아닌 다른 이유?"
   → [시기: 요즘] + [증상 조합: 코막힘+콧물X] + [궁금증: 다른 이유?]
   
✅ "겨울철 기침 오래가는데 열은 없어요 - 그냥 감기 맞을까요"
   → [시기: 겨울철] + [증상: 기침 오래+열X] + [의문: 감기 맞을까요]

✅ "목 불편한데 감기약 먹어도 안 나아요 - 왜 그럴까"
   → [증상: 목 불편+약 효과X] + [의문: 왜]

✅ "아침에 일어나면 어깨 뻣뻣하고 팔 올리기 힘들어요"
   → [시점: 아침] + [증상 조합: 어깨 뻣뻣+팔 올리기 힘듦] (검색어 그대로)
...
**💡 제목 자가 검증 (검색형 체크리스트):**
□ 네이버 검색창에 실제로 입력할 법한 문장인가?
□ "~인가요?", "~일까요?", "~해요", "왜", "어떻게" 같은 일상 표현 있나?
□ 증상이 구체적이고 조합되어 있나? ("A인데 B는")
□ 🚨 숫자가 하나도 없는가? (통계, 기간, 연령대 등)
□ 시기성 키워드 있나? (요즘, 겨울철, 환절기, 최근, 아침에)

**🎯 검색형 제목의 핵심: "사람이 실제로 물어보는 방식"**
...
✅ **증상/경험부터 직입:**
- 2문단 정의 파트 과감히 생략
- 바로 "아침마다 무릎이 불편한 이유는..." 같은 증상 이야기로 시작
...
❌ "오십견 관리 방법" → ✅ "불편함이 이어지는 과정에서"
❌ "통증 완화 치료" → ✅ "옷 갈아입을 때마다 불편할 때"
...
1. **🔍 설명형·상황 제시형 제목 작성 (질문형 금지!)**
   ...
   **예시:**
   ...
   ✅ "계단 내려가다 무릎에서 소리 난 뒤로 걸을 때마다 불편하다면" (상황 제시형)
...
❌ "스키장 부상 시 들린 무릎 소리, 붓기가 잘 빠지지 않는 흐름" (AI 냄새)
✅ "스키장에서 넘어진 뒤 무릎에서 소리가 났는데, 붓기가 안 빠질 때" (자연스러움)
...
- "밤에 누우면 어깨가 아파서 옆으로 못 누울 때" (시간 + 증상, 단정 없음)
...
`;
  ...
  // 🚀 v8.3 의료광고법 완전 준수 + 자연스러운 톤 강화
  const blogPrompt = `
너는 한국 병·의원 네이버 블로그용 의료 콘텐츠 에디터다.

[🚨 글자 수]
${targetLength + 100}자 목표 (최소 ${targetLength + 80}자, 최대 ${targetLength + 120}자)

[작성 요청]
진료과: ${request.category} / 주제: ${request.topic} / 이미지: ${targetImageCount}장
${learnedStyleInstruction || ''}${customSubheadingInstruction || ''}

[🚨 절대 금지 - 의료광고법]
❌ 단정: "치료된다", "낫는다", "효과 있다", "반드시", "~입니다"(단정형), "중요합니다", "필수입니다"
❌ 유도: "치료", "호전", "복귀", "도움", "좋다", "첫걸음", "개선"
❌ 권유: "~하세요", "방문하세요", "받으세요", "~하시기 바랍니다"
❌ 광고: "최고", "1등", "완치", "보장"
❌ 숫자: 모든 수치 표현 금지
   • 통계: "90%", "절반", "대부분의"
   • 기간: "2주", "3개월", "며칠", "하루", "1년"
   ...
   • 횟수: "3회", "몇 번", "여러 차례"
   • 시간: "10분", "30분", "하루"
   ✅ 대신: "드물지 않게", "경우에 따라", "시간이 지나면"
❌ 일반화: "모든", "누구나", "항상", "절대", "전부", "일반적으로", "보통", "흔히", "대부분"
   ❌ "모든 사람이 경험합니다" / "일반적으로 통증의 원인은..."
   ✅ "경험하는 경우가 있어요" / "통증이 있을 때는..."
❌ 의료용어: "의료", "의료진", "전문의", "의사", "진단", "처방", "검진", "증상"(추상어)
❌ 출처: 질병관리청, 보건복지부 등 기관명 금지
❌ 불안·강요: "위험하다", "심각하다", "방치하면", "~해야 한다", "~해야 합니다", "필요하다", "필요합니다"

[✅ 대체 표현]
단정 → 가능성:
"중요합니다" → "중요할 수 있어요" / "살펴볼 부분이에요"
"원인입니다" → "원인일 수 있어요" / "원인 중 하나예요"

숫자 → 정성:
"2주" → "시간이 지나면" / "어느 정도 시간이 흐르면"
"90%" → "많은 경우" / "자주"
"50대" → "나이가 들면" / "중년 이후"
"하루 3회" → "자주" / "여러 번"

일반화 → 개별:
"모든 사람이" → "경우에 따라" / "사람마다 다르지만"
"일반적으로" → "때로" / "경우에 따라"
"항상" → "때로" / "자주" / "종종"

[❌ AI 냄새 - 즉시 탈락]
1. 나열: "~인지, ~인지, ~인지" 3개 이상
2. 접속사: 같은 접속사 3회 이상 ("이처럼", "따라서", "그러므로" 등)
3. 추상어: "불편함", "상태" 과다 사용 (각 3회 미만)
4. 정의형: "~이란?" / "~에 대해 알아보겠습니다" 금지
5. 백과사전: "체크포인트", "확인사항" 소제목 금지
6. 메타 설명: "오늘은 ~알아볼까요?", "~살펴보겠습니다", "이 글은 ~" 금지
7. 병원 홍보: 설득·권유·안내 톤 금지 (중립적으로)

[✅ 사람처럼 쓰기 - 인간미 강화]
1. 감정 표현 (최소 3개):
   "걱정되는", "답답한", "불편한", "신경 쓰이는", "마음 한편으론", "조금은"
   
2. 공감 표현 (최소 2개):
   "그런 경험, 한 번쯤은 있으시죠?", "익숙한 상황일 거예요", "공감되는 부분이 있을 거예요"

⚠️ 중요: 개인 경험, 에피소드 금지
   ❌ '나', '집에서', '느낀 순간' 같은 체험형 문장 금지
   ✅ 관찰자 시점, 정보 제공 중심으로 작성

[✅ 네이버 SEO 최적화]
1. 제목 전략 (25자 내외):
   ⚠️ 진짜 아픈 사람들이 검색하는 제목
   - 정보 제공 목적, 의료·건강 정보성 톤
   - 차분하고 설명적인 문장
   - 기저질환자에게 주의가 필요한 이유가 드러나게
   - 과장, 공포 조장 표현 사용하지 않기
   
   ✅ 좋은 제목 예시:
   "무릎 통증, 기저질환자 주의가 필요한 이유"
   "기침 지속 시 확인해야 할 건강 신호"
   "어깨 통증과 관절 건강의 연관성"
...
2. 키워드 전략:
   - 주제 키워드("${request.topic}") 3~5회 자연스럽게 반복
   - 연관 키워드 1~2개 포함 (예: "무릎 통증" → "무릎 건강", "관절 관리")
   - 강제 반복 ❌, 흐름 속에 녹이기 ✅

[✅ 의료광고법 추가 안전장치]
1. 진단 암시 완전 차단:
   ❌ "원인일 수 있어요" → ✅ "이런 경우도 있어요"
   ❌ "~의 신호예요" → ✅ "~와 관련 있을 수 있어요"
...
[✅ 글쓰기 원칙]
1. 톤: 중립·담담·친근한 이웃 시점 (병원 홍보 ❌)
2. 문체: 설명 ❌ → 경험담 공유 ✅
   ❌ "무릎 통증의 원인은 여러 가지가 있습니다"
   ✅ "무릎이 아플 때는 여러 이유가 겹쳐 있을 수 있어요"
   
3. 전문 용어: 꼭 필요한 것만 (길게 설명 금지)
4. 독자 판단: 결론 강요 ❌, 선택 여지 ✅
5. 흐름: 나열식 ❌ → 이야기처럼 자연스럽게 ✅
6. 수술·시술: 언급 최소화 (불안 자극 금지)
7. 의학적 원리·일반론 삭제:
   ❌ "무릎 관절의 구조를 살펴보면..." → 삭제!
   ❌ "의학적으로 설명하면..." → 삭제!
   ✅ 대신 독자의 구체적 경험으로 대체
...
[✅ 글쓰기 핵심 3단계]
1. 도입 (2개 문단): 구체적 일상 + 공감
   ✅ "계단 내려갈 때마다 무릎이 '뚝' 소리와 함께 찌릿하게..."
   ✅ "그런 경험, 한 번쯤은 있으시죠?" (공감 유도)
   
2. 전개 (소제목당 2~3개 문단): 경험담→이유→사례
   - 첫 문단: 경험담 공유 (설명 ❌, 이야기 ✅)
   - 둘째 문단: 왜 그럴까 (의학 지식 ❌, 생활 경험 ✅)
   - 셋째 문단 (선택): 실제 사례 느낌
   - 감정 표현 3개: "걱정되는", "답답한", "불편한", "신경 쓰이는"
   - 감각 묘사 3개: "찌릿", "뻐근", "욱신", "뻣뻣", "묵직", "시큰"
   - 상황 구체화: "앉았다 일어날 때", "아침에 처음 걸을 때"
   
3. 마무리 (1개 문단): 담담한 마무리
   ✅ "몸이 보내는 신호를 알아차리는 것, 그것만으로도 의미가 있어요."

[✅ 리듬감]
1. 종결어미 순환 (연속 2회 금지):
   ~수 있어요 / ~곤 해요 / ~편이에요 / ~는 경우가 있습니다 / ~기도 합니다
   
2. 문장 길이 변주: 짧음(15~20자) → 중간(35~45자) → 짧음(20~25자)

3. 접속사 다양화: "한편", "더불어", "이와 달리", "다만" (같은 것 3회 금지)

4. 문단 길이: 2~4문장 (최대 5문장)
   - 모바일 화면 고려
   - 한 문단이 너무 길면 분리
...
1번: 상황형 ("~이유" / "~할 때")
2번: 질문형 ("왜 ~할까?" / "~않을까?")
3번: 실천형 ("일상 속 작은 변화" / "생활 속에서")

**키워드 배치:**
- 3개 소제목 중 최소 2개에 주제 키워드 포함
- 예: "무릎 통증의 흔한 이유" / "무릎 관리, 일상에서"

[HTML 구조]
<div class="naver-post-container">
  <p>도입 문단 1 - 구체적 상황 + 감각 묘사</p>
  <p>도입 문단 2 - 공감 표현 ("그런 경험, 있으시죠?")</p>
  
  <h3>상황형 소제목 1 ("~이유" / "~할 때")</h3>
  <p>문단 1 - 경험담 공유 (이야기체)</p>
  <p>문단 2 - 왜 그럴까 (생활 경험 기반)</p>
  <p>문단 3 - 실제 사례 느낌 (선택)</p>${targetImageCount >= 1 ? '[IMG_1]' : ''}
  
  <h3>질문형 소제목 2 ("왜 ~할까?" / "~않을까?")</h3>
  <p>문단 1 - 경험담 + 감정 표현</p>
  <p>문단 2 - 이유 (일상 관점)</p>${targetImageCount >= 2 ? '[IMG_2]' : ''}
  
  <h3>실천형 소제목 3 ("일상 속 작은 변화")</h3>
  <p>문단 1 - 생활 속 작은 시도</p>
  <p>문단 2 - 가능성 열어두기</p>${targetImageCount >= 3 ? '[IMG_3]' : ''}
...
{"title":"제목","content":"HTML","imagePrompts":["프롬프트"],"fact_check":{"fact_score":85,"safety_score":95,"conversion_score":80,"ai_smell_score":10,"verified_facts_count":5,"issues":[],"recommendations":[]}}
  ...
  /* 기존 상세 프롬프트 주석 처리 (API 타임아웃 방지)
    ...
    [AI 냄새 제거 핵심]
    - 원 숫자 (①②③④⑤) 금지
    - 연결어 과다 (그러나, 하지만, 그런데) → 5회 미만
    - 강조 부사 남용 (매우, 굉장히, 상당히) → 6회 미만
    - 종결어미 다양화: ~습니다, ~됩니다, ~편입니다, ~기도 합니다

    🚨 **숫자 표현 절대 금지! (맨 위 1번 규칙 참고)**
    모든 통계/횟수/기간/개수 금지 → "매우 다양한", "드물지 않게", "자주", "며칠" 사용
