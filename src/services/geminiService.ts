...
const getGPT52Prompt = () => {
  ...
  // 1. 공통 의료광고법 준수 프롬프트 (getMedicalSafetyPrompt 내용 직접 포함)
  const medicalSafetyPrompt = `
...
(의료광고법 나머지 내용은 너무 길어서 생략 - 필요시 getMedicalSafetyPrompt() 참조)
`;

  // 2. AI 냄새 제거 피드백 (사용자 요청사항)
  const aiFeedbackRules = `
████████████████████████████████████████████████████████████████████████████████
[🚨 AI 냄새 제거 최우선 규칙 - 품질 검사 기준]
████████████████████████████████████████████████████████████████████████████████

① 문장 리듬 단조로움 (22/25점)
**문제점:**
• 문단마다 '설명 문단 + 출처(연도) 언급 + 불릿포인트 리스트' 구조가 3회 연속으로 기계적으로 반복됨
• '~안내돼요', '~언급되는 내용이에요', '~기준입니다' 등 서술어가 딱딱하고 종결어미가 단조로움
• '질병관리청(2025)', '대한감염학회·질병관리청(2025)' 등 논문 인용 같은 어색한 출처 표기가 문맥을 끊음

**수정 제안:**
✅ 두 번째 문단(합병증)의 불릿포인트 리스트를 삭제하고, '열이 안 떨어지면서 숨까지 차다면 주의가 필요해요'처럼 대화체 줄글로 변경
✅ 반복되는 '(2025) 기준입니다' 문구를 삭제하고, '최근 지침을 살펴보면', '이번 겨울 유행 패턴은' 등으로 자연스럽게 순화
✅ 문장 끝을 '~해요', '~더라고요', '~이지 않을까요?' 등으로 다양하게 섞어서 리듬감 부여

② 판단 회피형 글쓰기 (15/20점) - 강화 버전
**문제점:**
• 모든 의학적 판단 근거를 '질병관리청 자료'로만 돌리며 저자의 목소리가 배제됨
• '~흐름이 됩니다', '~안내돼요'와 같은 피동형 표현이 과도하게 사용됨
• 단순히 출처를 나열하기보다, 해석과 맥락을 더해야 함

**수정 제안:**
✅ '질병관리청 자료를 보면 ~로 안내돼요'를 '고열과 함께 목이 아프다면 독감을 먼저 의심해볼 수 있는 타이밍이에요'로 변경 (저자 관점 추가)
✅ '단정할 수는 없습니다'를 '증상만으로는 헷갈릴 때가 많으니, 이럴 땐 기간을 체크해보세요'로 변경하여 행동 가이드 제시
✅ 소제목의 이모지를 모두 제거하고, '감기일까요? 독감일까요?'처럼 질문형 소제목으로 변경
✅ '마무리' 섹션을 과감히 삭제하고, 핵심 메시지를 마지막 문단이나 CTA에 자연스럽게 통합
✅ 중간 소제목 하나를 삭제하고 두 내용을 하나의 흐름으로 연결하여 문단 길이 조절

⑤ 가짜 공감 (8/10점)
**문제점:**
• '다음 날이 더 걱정되죠', '고민하게 됩니다' 등 누구나 할 수 있는 영혼 없는 공감 멘트
• 구체적인 고통(근육통, 오한 등)에 대한 묘사 없이 '몸은 뻐근해져요' 정도로 퉁침
• 진부한 공감 문구는 오히려 독자 몰입을 방해함

**수정 제안:**
✅ '걱정되죠'를 '아침에 아이 등원 준비를 해야 하는데 몸이 천근만근이라 난감했던 경험, 있으시죠?'로 구체화
✅ '몸은 뻐근해져요'를 '누가 온몸을 두들겨 팬 것처럼 근육통이 심하게 오더라고요'로 생생하게 수정
✅ '진료 기준으로 점검해두는 편이'를 '혼자 끙끙 앓기보다 전문가의 도움을 받아 컨디션을 빨리 회복하는 쪽을 권해드려요'로 순화
✅ 독자의 구체적인 상황을 상상하며, 그 상황에서 느낄 감정을 구체적으로 묘사
