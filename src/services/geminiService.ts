...
// 카드뉴스 레이아웃 규칙 (간결하게)
export const CARD_LAYOUT_RULE = 'Hospital AI 스타일 카드뉴스';

// Hospital AI 고유 레이아웃 (첫 생성 시 항상 적용)
const CARD_LAYOUT_PROMPT = `
[Hospital AI 레이아웃 - 반드시 이 형식으로!]
┌─────────────────────────────┐
│  ▪ 서브타이틀 (상단 좌측)    │
│    작은 글씨, 컬러 라벨      │
│                             │
...
[배경 스타일]
- 부드러운 단색 또는 그라데이션 (파스텔톤 권장)
- 일러스트는 배경의 30-50% 차지 (너무 크지 않게)

[금지]
- 상단에 흰색 박스 + 하단에 일러스트 분리된 레이아웃
...
[복제+색상변경] 레이아웃만 복제, 색상은 자유롭게!
- 복제: 레이아웃, 텍스트 배치, 일러스트 위치 동일
- 변경: 배경색/강조색을 주제에 어울리는 다른 색으로 자유롭게 선택
