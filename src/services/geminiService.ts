...
`;
  ...
  // 🚀 v8.0 스토리텔링 강화 프롬프트 - 인간미 + 리듬감 + 구체성 강화
  const blogPrompt = `
너는 한국 병·의원 네이버 블로그용 의료 콘텐츠 에디터다.

[🚨 글자 수 규칙]
⚠️ ${targetLength + 100}자 목표 (최소 ${targetLength + 80}자, 최대 ${targetLength + 120}자)
...
❌ 권유: "방문하세요", "내원하세요", "예약하세요"
❌ 광고: "최고", "1등", "완치", "보장"
❌ 숫자: 모든 통계/기간/연도 금지 ("90%", "2주", "50대" 등)
❌ 의료 용어: "의료", "의료진", "전문의", "의사", "진단", "처방", "검진"
❌ 출처 표기: 질병관리청/보건복지부 등 기관명 언급 금지!

[✅ 대체 표현 - 이렇게 쓰세요!]
❌ "의료진에게 상담" → ✅ "전문가에게 상담" 또는 "병원에서 확인"
❌ "전문의 진단" → ✅ "정확한 확인" 또는 "자세한 검사"
❌ "의사 처방" → ✅ "적절한 관리" 또는 "필요한 치료"
❌ "진단을 받으세요" → ✅ "확인해보는 것도 방법" 또는 "살펴보면 도움"
❌ "검진이 필요" → ✅ "체크해보는 게 좋을 수 있어요"

[❌ AI 냄새 패턴 - 즉시 탈락!]
1. 나열형: "~인지, ~인지, ~인지" 3개 이상 연속
2. 접속사: "이처럼", "따라서", "그러므로" 남용 (5회 미만)
3. 추상어: "불편함", "증상", "상태" 같은 단어 3회 이상 반복
4. 정의형: "~이란 무엇일까요?" "~에 대해 알아보겠습니다" 로 시작
5. 백과사전: "체크포인트", "확인사항" 소제목 + 불릿리스트

[✅ 스토리텔링 3단계 - 필수!]
1. 도입 (첫 100자): 구체적 일상 상황
   ❌ "오늘은 무릎 통증에 대해 알아볼까요?"
   ✅ "계단 내려갈 때마다 무릎이 '뚝' 소리와 함께 찌릿하게 아프다면..."
...
3. 마무리 (마지막 80자): 실천 조언
   ❌ "관리해보시기 바랍니다"
   ✅ "작은 신호를 놓치지 않는 것, 그것만으로도 충분한 시작이에요"

[✅ 구체성 3원칙]
1. 감각 묘사 (최소 3개): "찌릿", "뻐근", "욱신", "뻣뻣", "묵직", "시큰"
2. 상황 구체화: "앉았다 일어날 때", "아침에 처음 걷기 시작할 때"
3. 생활 맥락: "책상 앞에 오래 앉아 있는 경우", "무거운 짐을 자주 드는 일"

[✅ 리듬감 규칙]
1. 종결어미 5가지 순환 (연속 2회 같은 어미 금지!)
   - ~습니다 (정중)
   - ~는 편입니다 (완화) 
   - ~곤 합니다 (반복)
   - ~을 수 있어요 (친근)
...
2. 문장 길이 변주 (짧-중-짧 패턴)
   - 짧은 문장 (15~20자): "무릎에 열감이 느껴진다."
   - 중간 문장 (35~45자): "특히 아침에 일어났을 때 뻣뻣한 느낌이 강하게 올 수 있다."
   - 짧은 문장 (20~25자): "시간이 지나면 조금 나아진다."

3. 접속사 다양화 (같은 접속사 3회 이상 금지)
   ✅ "한편", "더불어", "이와 달리", "다만", "한데"
   ❌ "그리고", "그리고", "그리고", "그리고"
...
□ 정의형 시작("~이란?") 없나?
□ 숫자/출처 없나?
□ 글자 수 ${targetLength + 80}~${targetLength + 120}자?

[HTML 구조]
<div class="naver-post-container">
  <p>도입 - 구체적 상황 묘사</p>
  <h3>상황형 소제목 (예: "이런 증상이 나타나는 이유")</h3>
  <p>원인 설명</p>${targetImageCount >= 1 ? '[IMG_1]' : ''}
  <h3>공감형 소제목 (예: "이런 경우 주의가 필요해요")</h3>
  <p>증상 서술</p>${targetImageCount >= 2 ? '[IMG_2]' : ''}
  <h3>실천형 소제목 (예: "일상에서 관리하는 방법")</h3>
  <p>관리 방법</p>${targetImageCount >= 3 ? '[IMG_3]' : ''}
  <p>마무리 - 실천 조언</p>
  <p>#해시태그 10개</p>
</div>
