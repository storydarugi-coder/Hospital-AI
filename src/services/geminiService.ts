...
import { GenerationRequest, GeneratedContent, TrendingItem, FactCheckReport, SeoScoreReport, SeoTitleItem, ImageStyle, WritingStyle, CardPromptData, CardNewsScript, CardNewsSlideScript, ContentCategory } from "../types";
import { getStagePrompt, getSystemPrompt, getFullPrompt, DEPARTMENT_PROMPTS, getDepartmentPhrases } from "../lib/gpt52-prompts-staged";
...
const callGemini = async (prompt: string, systemPrompt?: string): Promise<string> => {
  try {
    ...
    console.error('❌ callGemini 에러:', error);
    throw error;
  }
};
...
const getMedicalSafetyPrompt = () => {
  ...
  return `
당신은 대한민국 의료광고법·네이버 정책을 준수하는 병원 블로그 에디터입니다.

🚨 [3대 핵심 원칙]
1. 병명/증상 정의 금지: "OO란 ~이다" (X) → "OO가 언급되곤 합니다" (O) [정보성 유지]
2. 판단/진단 회피: "의심해봐야" (X) → "살펴보는 것이 좋습니다" (O) [의료행위 오해 방지]
3. 결과 보장 금지: "완치/해결/회복" (X) → "도움이 될 수 있습니다" (O) [개인차 명시]

🚫 [절대 금지 표현]
- 최상급/보장: 최고, 유일, 100%, 완치, 기적, 특효, 무조건
- 공포 조장: 방치하면 사망, 골든타임, 즉시, 긴급, 위험
- 직접 유도: 오세요, 방문하세요, 상담하세요, 전화주세요 (병원명/위치 노출 금지)
- 수치 단정: 3일 내 회복, 48시간 잠복기 (→ "개인차 있음"으로 대체)

✅ [안전한 작성 패턴]
- 설명형보다 상황 묘사: "증상은 ~입니다" (X) → "~한 불편함을 호소하기도 합니다" (O)
- 인과관계 완화: "~때문입니다" (X) → "~관련이 깊은 것으로 알려져 있습니다" (O)
- 제안형 마무리: "검사하세요" (X) → "상태를 점검해보는 계기로 삼는 것도 방법입니다" (O)
`;
};
...
const getAIFeedbackPrompt = (): string => {
  return `
---
[🤖 AI 냄새 제거 + 의료광고법 준수 글쓰기]
---

**❌ 금지 1: "~수 있습니다" 연속 3회 이상 금지!**
- "도움이 될 수 있습니다", "중요할 수 있습니다", "필요할 수 있습니다"
✅ 대체: "~한 경우가 있습니다", "~으로 언급되곤 합니다"

**❌ 금지 2: 교과서식 구조 (정의→원인→증상→치료) - 병원 홈페이지 전형!**
✅ 대체: 상황 묘사 → 증상 흐름 → 참고 상황 → 마무리

**❌ 금지 3: 정의형 시작 - 의료광고 필터 경고!**
- "OO이란 ~을 의미합니다", "OO은 ~한 질환입니다"
- "OO란 무엇인가요?"
✅ 대체: "이 시기에 자주 거론되는 것 중 하나가 OO입니다"
→ 병명은 정의하지 말고 맥락 속에서만 등장!

**❌ 금지 4: 가능성·판단 반복**
- "OO 가능성을 고려해볼 수 있는", "판단하는 데 참고가"
→ 의심·가능성 반복 = 네이버 광고 신호 누적!
✅ 대체: "~한 상황이 함께 언급되곤 합니다"

**❌ 금지 5: 추상 명사 연결**
- "기준을 세우다", "판단이 정리되다", "도움이 되다" → 추상적!
✅ 대체: 구체적 상황 제시
  "이런 패턴이 반복된다면 경과를 살펴보는 것도 방법입니다"

**❌ 금지 6: 숫자·기간 표현**
- "24~48시간", "2~3일이면 회복"
✅ 대체: "개인에 따라 차이가 있습니다", "사람마다 다를 수 있습니다"
...
// =============================================
// 🚀 시스템 프롬프트 캐싱 시스템
// - 토큰 절약: 반복되는 시스템 프롬프트를 메모리에 캐싱
// - Gemini는 동일 프롬프트 시 서버 측 캐싱 활용
// =============================================

// 압축된 핵심 시스템 프롬프트 (토큰 최적화)
const CACHED_SYSTEM_PROMPT = `당신은 의료광고법 준수 네이버 병원 블로그 전문 에디터입니다.

[핵심 규칙 - 필수 준수]
1. 금지어: 가능성/판단/의심/체크/진단/효과/완치/성공률
2. 숫자/기간 → "개인차" 표현으로 대체
3. 정의형 문장 금지 ("OO란~" → 맥락 속 언급)
4. "~수 있습니다" 3회 연속 금지 → 다양한 종결어미
5. 마무리: 직접 권유 대신 선택형 제안

[안전한 표현 패턴]
✅ "이런 경우도 있습니다" / "개인차가 있습니다"
✅ "상황에 따라 달라질 수 있습니다"
✅ "궁금한 점이 있으시다면 확인해보시는 것도 방법입니다"
...
[JSON 응답 형식]
{
  ...
  "content": "HTML 본문",
  "imagePrompts": ["프롬프트1", ...],
  "fact_check": { "fact_score": 85, "safety_score": 95, "conversion_score": 80, "ai_smell_score": 7, "verified_facts_count": 5, "issues": [], "recommendations": [] }
}`;

// 캐시된 프롬프트 인스턴스 (메모리 캐싱)
let _cachedPromptInstance: string | null = null;
...
// 캐시된 시스템 프롬프트 가져오기
const getCachedSystemPrompt = (category?: ContentCategory): string => {
  const categoryPrompt = category ? getSystemPrompt(category) : '';
  const fullPrompt = `${CACHED_SYSTEM_PROMPT}\n\n${categoryPrompt}`;
  const hash = hashPrompt(fullPrompt);
  
  if (_cachedPromptHash === hash && _cachedPromptInstance) {
    console.log('📦 시스템 프롬프트 캐시 히트');
    return _cachedPromptInstance;
  }
  
  console.log('📦 시스템 프롬프트 캐시 미스 - 새로 생성');
  _cachedPromptInstance = fullPrompt;
  _cachedPromptHash = hash;
  return fullPrompt;
};
...
올바른 예:
✅ "구토가 반복되고 물을 마셔도 유지하기 어려운 상태가 이어질 수 있다. 설사가 물처럼 나오면서 탈수로 이어질 위험도 있다. 주변에서도 비슷한 증상이 이어진다면 전염 가능성을 고려해야 한다."
...
export const recommendSeoTitles = async (topic: string, keywords: string, postType: 'blog' | 'card_news' = 'blog'): Promise<SeoTitleItem[]> => {
  ...
  const prompt = `전문 의료 마케터로서 "${topic}"(${keywords}) 주제의 ${postType === 'card_news' ? '카드뉴스' : '블로그'} 제목 5개를 제안하세요.

[현재: ${currentYear}년 ${currentSeason}]
[원칙]
1. 의료법 준수: 치료/완치/효과/최고/유일/강력/기적/보장 금지. "해결/극복" 등 결과 암시 금지.
2. 낚시성/공포 금지: "충격", "경악", "사망 위험", "골든타임" 금지.
3. 질문형 자제: 네이버 로직상 "~일까요?"보다 "~한 경우", "~신호"가 유리.
4. AI 냄새 제거: "과정/흐름/시점/사례" 등 기계적 어미 대신 "순간/때/경우/신호" 사용.

[SEO 구조]
- 키워드를 제목 앞쪽(50%)에 배치.
- 클릭을 유도하되 내용은 담백하게.
- 유형: 호기심(궁금증), 공감(내 얘기), 정보(유용성), 시기(제철), 맥락(원인).

[출력 포맷]
JSON: [{ "title": "...", "score": 85, "type": "공감" }, ...]`;
      ...
      responseSchema: {
        ...
        items: {
          ...
          properties: {
            ...
            score: { type: Type.NUMBER },
            type: { type: Type.STRING, enum: ['호기심', '공감', '정보전달', '시기고정', '맥락설명'] }
          },
...
const storyPlannerAgent = async (
  ...
  const prompt = `당신은 **전환형 카드뉴스** 스토리 기획자입니다.
[미션] "${topic}" 주제로 ${slideCount}장 카드뉴스 기획 (${category}, ${writingStyle === 'expert' ? '전문가형' : writingStyle === 'empathy' ? '공감형' : '전환형'})

🚨 [카드뉴스 핵심: "읽기"가 아니라 "판단"하게 하라!]
- 설명(Description)은 1줄(25자) 이내로 제한. 긴 설명은 스크롤을 멈추게 하지 못함.
- 독자가 "어? 내 얘기네?" → "확인해봐야겠네"로 마음이 바뀌는 흐름 설계.

[카드별 역할 (${slideCount}장)]
1. 표지: 부제(상황) + 제목(질문/공감). ⚠️설명(Description) 없음!
2. 문제제기: 잘못된 상식 깨기 or "단순한 게 아닐 수 있다"는 의문.
3~${slideCount-2}. 증상/공감: 구체적 상황 묘사 ("이런 느낌 드나요?"). 판단은 1줄로.
${slideCount-1}. 시점고정: "반복된다면 확인이 필요한 때입니다". (강요 금지)
${slideCount}. CTA: "지금이 점검할 타이밍". ⚠️설명(Description) 없음!

[텍스트 규칙]
- subtitle: 4~8자 (질문/상황).
- mainTitle: 10~15자 (핵심). <highlight>강조</highlight> 사용.
- description: 15~25자 (판단 1줄). 표지와 마지막장은 비워둠("").
- 의료법: "완치/보장/최고/방문하세요/상담하세요" 절대 금지.
- CTA: "미루면 손해" or "지금이 적기" 심리 기법 사용. 명령조(~하세요) 금지.

[출력 필드]
- topic, totalSlides, overallTheme(한국어), slides 배열.
`;
...
`;
  ...
  // 🆕 진료과별 동적 프롬프트 데이터 가져오기
  const departmentData = DEPARTMENT_PROMPTS[request.category as ContentCategory];
  const departmentPhrases = getDepartmentPhrases(request.category as ContentCategory);
...
[🎯 ${departmentData?.name || request.category} 맞춤 글쓰기 가이드]
📌 자주 다루는 주제: ${departmentData?.commonSymptoms?.join(', ') || '일반 건강 정보'}
📌 상황 묘사 표현 참고:
${departmentPhrases || '• 일상에서 겪는 불편함을 구체적으로 묘사\n• 독자가 "내 얘기네" 느끼도록 서술'}
📌 CTA 스타일: ${departmentData?.ctaStyle || '전문적인 확인을 받아보는 것도 방법이다'}
...
⚠️ "~의심해볼 수 있다" → ✅ "~가능성도 하나의 원인으로 고려해볼 수 있는 상황"
...
[🏥 병원 블로그 정체성 - 은근히 드러내기]
12. 치료/관리 섹션 첫 문단 아래에 현장감 문장 1개 삽입:
    예) "실제 진료 현장에서도 ~으로 넘겼다가 증상이 악화된 상태로 상담을 받는 경우를 종종 보게 됩니다."
    → 병원 홍보❌, 의료광고❌, "아 여기 병원이구나" 인식⭕

[📝 마무리 문단 - 행동의 씨앗 심기]
13. 마지막 문단은 "좋은 말"로 끝내지 말고 행동 유도:
...
<div class="naver-post-container">
  ...
  <p>"다만 이는 참고용이며, 정확한 판단은 전문가의 진료가 필요합니다."</p>
  
  <h3>${request.topic} 관리 및 완화 방법은?</h3>
  <p>비수술적 방법 우선 설명</p>
  <p>🏥 현장감 문장: "실제 진료 현장에서도 ~한 경우를 종종 보게 됩니다."</p>
...
반드시 JSON 형식으로 응답하고, 목표 길이(공백 제외 ${targetLength}자)를 준수해주세요.
  ...
  /* 기존 상세 프롬프트 주석 처리 (API 타임아웃 방지)
    ...
    [🚨 글자 수 - 가장 중요한 요구사항!!! 🚨]
    ⚠️ 목표: 공백 제외 **최소 ${targetLength}자 이상** 필수!!! ⚠️
    
    [금지] **절대 금지:** ${targetLength}자보다 적게 쓰는 것! (1450자 같은 분량 미달은 실패!)
...
[JSON 응답 형식]
{
  ...
  "latest_guidelines": [
    ...
    if (geminiResults && gptResults) {
      ...
      safeProgress('🔀 크로스체크: Gemini + Gemini 결과 병합 중...');
      ...
      console.log(`   🟢 Gemini: ${gptTotal}개 정보`);
    ...
    // 📍 Step 2: Gemini가 검색 결과를 바탕으로 글 작성
    // 📍 Step 2: Gemini 글쓰기
    console.log('📍 Step 2 시작: Gemini 글쓰기...');
    if (typeof onProgress === 'function') {
      safeProgress('✍️ Step 2: Gemini가 자연스러운 글을 작성하고 있습니다...');
    }
...
4. 최신 정보를 적극 반영하되, 검증된 정보를 우선`;
    ...
    // Gemini 사용 + 시스템 프롬프트 캐싱
    console.log('🔵 Using Gemini for text generation (with cached system prompt)');
    const cachedSystemPrompt = getCachedSystemPrompt(request.category);
    console.log('📏 시스템 프롬프트 길이:', cachedSystemPrompt.length, 'chars');
    console.log('📏 사용자 프롬프트 길이:', (isCardNews ? cardNewsPrompt : blogPrompt).length, 'chars');
    safeProgress('✍️ Gemini가 콘텐츠를 작성하고 있습니다...');
    
    try {
      console.log('🔄 Gemini API 호출 시작 (캐시된 시스템 프롬프트 사용)...');
      console.log('📦 contextData 길이:', contextData?.length || 0);
      console.log('📦 blogPrompt 길이:', blogPrompt?.length || 0);
      
      // 🎬 캐시된 시스템 프롬프트 + 사용자 프롬프트 분리
      safeProgress('✍️ AI가 콘텐츠를 작성하고 있습니다... (잠시만 기다려주세요)');
      
      const geminiResponse = await ai.models.generateContent({
        model: "gemini-3-pro-preview",
        // 시스템 프롬프트와 사용자 프롬프트 분리 (캐싱 최적화)
...
[출력 형식 - JSON]
{
  ...
  "changes_made": ["변경한 내용 1", "변경한 내용 2", ...]
      ...
      const improvedAiText = await callGemini(aiSmellImprovementPrompt, 'AI 냄새 제거 전문가로서 사람이 쓴 것처럼 자연스럽게 개선해주세요.');
