...
CTA의 목적은 '방문 권유'가 아니라 **'다른 선택지를 지워주고 + 시간을 못 박는 것'**입니다.
독자 머릿속의 "그럼… 오늘은 아니고, 조만간?" 이 망설임을 제거해야 합니다.

**[CTA 3대 핵심 장치 - 반드시 조합해서 사용]**

1. **⏰ 시간 조건 (가장 강력!) - '나중'이라는 선택지 제거**
   - "언제"를 명시하면 독자는 미룰 수 없음
   - 의료법 100% 준수하면서 결단 압박 가능
   
   ✅ 시간 조건 표현:
   - "이런 증상이 **갑자기** 나타났다면" → 급성 느낌
   - "**당일 기준으로** 확인하는 것이 우선입니다" → 오늘 행동 유도
   - "증상이 시작된 **시점에서** 확인하느냐, 미루느냐가 결과를 가릅니다"
   - "**이 시점부터는** 지켜보기보다 확인이 필요합니다"
   - "**48시간 이내**에 악화될 수 있어서"
   
   ❌ 시간 조건 없는 약한 CTA:
   - "증상이 심하면 병원 진료를 고려해 보세요" → 언제? 내일? 다음 주?

2. **🔒 비가역성 (되돌릴 수 없음) - 결정의 무게 부여**
   - 공포가 아니라 **의학적 사실**로 결단 유도
   - 본문에서 언급한 "회복 어려움"을 CTA와 직결
   
   ✅ 비가역성 + CTA 조합:
   - "OO은 한 번 손상되면 회복이 어렵기 때문에, 증상이 시작된 시점에서 확인하느냐 미루느냐가 결과를 가릅니다"
   - "초기에 놓치면 되돌리기 어려운 경우가 많아서, 이 단계에서는 지켜보기보다 확인이 우선입니다"
   - "골든타임을 지나면 치료 옵션이 줄어들기 때문에, 증상이 나타난 시점에서 바로 확인하는 게 순서입니다"
   
   ⚠️ 주의: 과장/공포 유발 금지 (의료법 준수)
   - ❌ "지금 안 하면 실명합니다" → 과장
   - ✅ "회복이 어려운 경우가 많아서" → 의학적 사실

3. **🚫 배제형 구조 - 다른 선택지 제거**
   - "이 단계에서는 ○○이 아니라 △△가 우선입니다" 공식
   
   ✅ 배제형 CTA 완성형 (시간+비가역성+배제 조합):
   - "이런 증상이 갑자기 나타났다면, 인공눈물로 지켜볼 단계는 이미 지났고, 당일 기준으로 혈관 상태를 확인하는 것이 우선입니다"
   - "이 단계에서는 휴식이나 인공눈물이 아니라, 망막 혈관 상태를 직접 확인하는 검사가 우선입니다"
   - "한 번 손상되면 회복이 어렵기 때문에, 자가 판단으로 약을 고르는 것보다 원인을 구분하는 진료가 먼저입니다"

**[CTA 위치 전략 - 중간 + 마지막]**

4. **📍 중간 CTA (각 파트 끝에 삽입) - 가장 실무에서 잘 먹힘**
   - 독자가 읽다가 멈추고 생각하게 만드는 CTA
   - 광고처럼 안 보이면서 행동 유도
   
   ✅ 중간 CTA 삽입 위치와 예시:
   - **골든타임/위험성 파트 끝**: "이 시점부터는 증상을 비교하는 것보다 OO 상태를 직접 확인하는 검사가 치료의 출발점이 됩니다"
   - **원인 파트 끝**: "원인이 다양하기 때문에, 자가 판단보다는 정확한 검사로 원인을 구분하는 게 먼저입니다"
   - **증상 파트 끝**: "이런 증상이 겹친다면, 일반 관리로 넘어가기보다 당일 기준으로 확인하는 게 순서입니다"

5. **🎯 마무리 CTA - 최종 결단**
   - 감성 마무리 ❌ → 판단 문장으로 마무리
   
   ✅ 강한 마무리 (시간+비가역성+배제 조합):
   - "초기에 확인하느냐, 미루느냐가 치료 결과를 가릅니다. 증상이 시작됐다면, 지금이 확인할 타이밍입니다"
   - "되돌리기 어려운 경우가 많기 때문에, 이 단계에서는 지켜보기보다 검사가 우선입니다"
...
6. **예방/관리는 30%로 압축**
   - 검사/진단이 주인공, 생활습관은 조연
   - 핵심 2-3개만 → 바로 "하지만 이미 증상이 있다면..." CTA로 연결
...
// 시간조건 + 비가역성 + 배제형 CTA 공식 (의료광고법 100% 준수)
...
1. **⏰ 시간 조건** - "나중"이라는 선택지 제거
   - "갑자기 나타났다면" / "당일 기준으로" / "이 시점부터는" / "48시간 이내에"
   
2. **🔒 비가역성** - 되돌릴 수 없음을 의학적으로 제시 (공포X, 사실O)
   - "한 번 손상되면 회복이 어렵기 때문에"
   - "골든타임을 지나면 치료 옵션이 줄어들기 때문에"
   - "초기에 놓치면 되돌리기 어려운 경우가 많아서"
...
- "이런 증상이 갑자기 나타났다면, 인공눈물로 지켜볼 단계는 이미 지났고, 당일 기준으로 혈관 상태를 확인하는 것이 우선입니다"
- "OO은 한 번 손상되면 회복이 어렵기 때문에, 증상이 시작된 시점에서 확인하느냐 미루느냐가 결과를 가릅니다"
- "골든타임이 있기 때문에, 이 단계에서는 지켜보기보다 당일 확인이 치료의 출발점입니다"

✅ 중간 CTA (본문 각 파트 끝에 삽입):
- 골든타임 파트 끝: "이 시점부터는 증상을 비교하는 것보다 OO 상태를 직접 확인하는 검사가 치료의 출발점이 됩니다"
- 위험성 파트 끝: "되돌리기 어려운 경우가 많기 때문에, 자가 판단보다는 원인을 구분하는 진료가 먼저입니다"
