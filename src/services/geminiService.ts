...
`;
  ...
  // 현재 계절에 맞는 도입부 예시 동적 생성
  const getSeasonalIntroExamples = (month: number) => {
    if (month === 12 || month === 1 || month === 2) {
      // 겨울 (12, 1, 2월)
      if (month === 1) {
        return `  • 현재 시기 예시: "새해 겨울철 차가운 공기 때문에 몸이 더 움츠러드는 요즘"
  • 현재 시기 예시: "겨울철 건조한 날씨 때문에 피부도 당기고 목도 칼칼한 요즘"`;
      } else if (month === 2) {
        return `  • 현재 시기 예시: "겨울 끝자락 환절기가 시작되는 요즘, 찬바람이 여전한데"
  • 현재 시기 예시: "아직 추위가 가시지 않은 요즘, 아침저녁으로 쌀쌀한"`;
      } else { // 12월
        return `  • 현재 시기 예시: "한겨울 추위가 절정인 요즘, 건조한 날씨에"
  • 현재 시기 예시: "겨울 초입 연말, 난방기 사용이 많아지는 요즘"`;
      }
    } else if (month >= 3 && month <= 5) {
      // 봄 (3, 4, 5월)
      return `  • 현재 시기 예시: "환절기 따뜻해지는 봄철, 미세먼지와 꽃가루가 날리는 요즘"
  • 현재 시기 예시: "봄철 일교차가 큰 요즘, 아침저녁으로 쌀쌀한"
  • 현재 시기 예시: "따뜻한 봄 날씨에 야외활동이 많아지는 요즘"`;
    } else if (month >= 6 && month <= 8) {
      // 여름 (6, 7, 8월)
      return `  • 현재 시기 예시: "무더운 여름철, 습한 날씨에 땀이 많이 나는 요즘"
  • 현재 시기 예시: "여름철 뜨거운 햇볕 아래, 냉방기가 가동되는 요즘"
  • 현재 시기 예시: "장마철 습한 날씨가 계속되는 요즘"`;
    } else {
      // 가을 (9, 10, 11월)
      return `  • 현재 시기 예시: "선선한 가을철, 큰 일교차 때문에 몸이 적응하기 힘든 요즘"
  • 현재 시기 예시: "환절기 가을, 아침저녁으로 쌀쌀해지는 요즘"
  • 현재 시기 예시: "가을철 건조한 날씨에 피부도 당기는 요즘"`;
    }
  };
  
  const seasonalIntroExamples = getSeasonalIntroExamples(currentMonth);
...
[글자 수 엄수] 목표: ${targetLength}자 (공백 제외)
