...
// 카드뉴스 레이아웃 규칙 (간결하게)
export const CARD_LAYOUT_RULE = '일러스트 배경 + 텍스트 오버레이 카드뉴스';

// Hospital AI 고정 레이아웃 (항상 사용)
const CARD_LAYOUT_PROMPT = `
[레이아웃 - 반드시 이대로!]
1. 배경: 부드러운 그라데이션 + 주제 관련 일러스트 (화면 70% 이하)
2. 상단: 작은 서브타이틀 (파란색/흰색 라벨)
3. 중앙: 메인 타이틀 (큰 검은 글씨, 반투명 흰색 배경)
4. 하단: 설명문 (작은 회색 글씨) - 표지/마지막 장은 생략
5. 일러스트 위치: 중앙 또는 하단에 자연스럽게 배치

[금지]
- 상단 흰색 박스 + 하단 일러스트 분리 레이아웃
- 영어 텍스트, 해시태그, 워터마크
