...
검색 순서 (반드시 이 순서대로):
1순위: health.kdca.go.kr (질병관리청 건강정보포털) - 최우선!
2순위: kdca.go.kr, mohw.go.kr (보건복지부), nhis.or.kr, hira.or.kr
3순위: *.or.kr (대한OO학회만), mfds.go.kr
4순위: who.int, cdc.gov, nih.gov, pubmed.ncbi.nlm.nih.gov, jamanetwork.com

검색 전략: "주제 + site:도메인" 형식 사용
금지: 블로그, 카페, SNS, 유튜브, 개인 사이트
...
1순위:1순위: 질병관리청 건강정보포털 (반드시 제일 먼저!)**
...
3순위:3순위: 해외 공신력 사이트 (국내 정보 부족 시에만!)**
...
❌ 제목: 물음표 금지! (소제목은 허용)
❌ 소제목: 이모지 절대 금지! (🎯, 📌, ⚠️, ✅, 🦴, 💊 등 모든 이모지)
✅ 소제목: 질문형 또는 구어체로 (예: "이런 증상이 반복되면 위험 신호일까?")

**✅ 제목 공식:** [시기성] + [일상 증상] + [의심 프레임] + [확인 기준]
예: "요즘 당기고 따가운 피부, 단순 건조함인지 확인이 필요한 신호들"

**⛔ 도입부/본문 금지:**
❌ "안녕하세요. ~에디터입니다", "오늘은 ~에 대해 알아보겠습니다"
❌ 숫자 표현 (출처 없으면): "15분 이상" → "충분히 압박해도"
...
✅ 대체: "도움이 됩니다", "중요합니다", "~하는 게 좋아요"

**❌ 금지 2: 교과서식 구조 (정의→원인→증상→치료)**
✅ 대체: 상황 묘사 → 의심 신호 → 확인 기준 → 다음 선택
...
**❌ 금지 4: 관찰자 시점 부족**
- 모든 문장이 설명만 하면 딱딱함
✅ 각 문단에 관찰자 시점 문장 1개 이상:
  "이런 경험 있으신가요?", "비슷한 고민을 가진 분들이 많더라고요"
...
5. 질환명은 단정하지 말고 가능성 구조 사용
   ✅ "~증상일 수 있는"
   ✅ "~의심될 수 있는"
   ✅ "~구분이 필요한"
   ❌ "당신은 ~입니다" (단정 금지)

6. 계절·상황·행동 맥락을 포함해 검색 의도 정확히 겨냥
   ✅ 겨울철, 아침, 반복되는, 씻고 나서, 밤마다, 식후에 등

7. 메인 키워드는 제목 앞부분에 배치 (네이버 SEO 기준)
   ✅ "피부건조 원인, 겨울철에 심해지는 이유"
   ❌ "겨울철에 심해지는 피부건조 원인"

8. 제목 길이: ${lengthGuide}

9. 감정 어휘는 사용하되 자극적으로 쓰지 않는다
   ✅ 답답한, 찝찝한, 불편한, 반복되는 (허용)
   ❌ 무섭다, 위험하다, 심각하다 (금지)
...
5. 원인-결과 인과관계 단정 금지 → "겹쳐 나타날 수 있음", "함께 고려되는 경우" 등 완충 표현 사용
6. 빈도·증가·유행 표현 제한 → "많다/늘었다/흔하다" 대신 "인식되는 경우", "느끼는 분들" 등 체감 표현으로 치환
7. 마지막 문단은 지시형이 아닌 선택형 정리 → "점검해볼 수 있다", "하나의 기준이 될 수 있다"
8. 병명은 설명 목적에 한해 제한적 사용 → 반복 노출이나 강조 금지

[🏆 1위 블로그 작성법 - 필수!]
9. 도입부는 독자 일상 체험형으로 3문단 구성 → "처음에는" → "점점" → "어느 순간" 시간 흐름으로 증상 진행 묘사
10. 소제목은 질문형(H3)으로 작성 → "~이란?", "~증상은?", "~방법은?" (검색 의도 정확 매칭)
11. 완충 표현 적극 사용 → "~경우가 있습니다"(다수), "~알려져 있습니다", "~도움이 될 수 있습니다"
12. 구체적 숫자/시간 포함 → "40~60분마다", "약 1분간" (추상적 표현 금지)
13. 단락은 3~4줄 단위로 끊기 → 시각적 피로 최소화, 짧은 문장과 긴 문장 믹스
14. 문장 종결 다양화 → "~입니다" 연속 사용 금지, "~습니다", "~됩니다", "~있습니다" 교차 사용

[HTML 구조 - 1위 글 스타일]
<div class="naver-post-container">
  <p>안녕하세요. [지역] [진료과] '[병원명]'입니다.</p>
  <p>도입 1문단: 독자 일상 체험 묘사 (3~4줄, "처음에는")</p>
  <p>도입 2문단: 증상 진행 과정 (3~4줄, "점점" "어느 순간")</p>
  <p>도입 3문단: 병명 자연스럽게 도입 (2~3줄, "이와 같은 상태를 일반적으로...")</p>
  
  <h3>${request.topic} 이란?</h3>
  <p>개념 설명 (3~4줄 단락 × 2~3개)</p>
  <p>구체적 원인/배경 (완충 표현 사용)</p>
  ${targetImageCount >= 1 ? '\n  [IMG_1]\n' : ''}
  
  <h3>${request.topic}의 주요 증상은?</h3>
  <p>증상 설명 (3~4줄 단락 × 2~3개)</p>
  <p>시간대별/상황별 증상 차이</p>
  ${targetImageCount >= 2 ? '\n  [IMG_2]\n' : ''}
  
  <h3>${request.topic} 자가 진단 방법은?</h3>
  <p>독자가 직접 해볼 수 있는 방법 (구체적 숫자 포함)</p>
  <p>"다만 이는 참고용이며, 정확한 판단은 전문가의 진료가 필요합니다."</p>
  
  <h3>${request.topic} 치료/관리 방법은?</h3>
  <p>비수술적 방법 우선 설명 (3~4줄 단락 × 3~4개)</p>
  <p>각 방법의 목적과 적용 상황 (완충 표현)</p>
  ${targetImageCount >= 3 ? '\n  [IMG_3]\n' : ''}
  
  <h3>${request.topic} 예방 방법은?</h3>
  <p>실천 가능한 구체적 방법 (숫자 포함)</p>
  <p>선택형 마무리: "주의 깊게 관찰하는 것이 필요합니다" (행동 지시 금지)</p>
...
4순위:AI 냄새 점수 (ai_smell_score)**: 0~100점 (낮을수록 좋음)
- 문장 리듬이 단조로운가? (0~25점)
