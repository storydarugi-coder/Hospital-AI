...
2. 🚨🚨🚨 번역투 표현 절대 금지 (자연스러운 한국어로 수정):
   ❌ 완전 금지 표현:
   • "~와 연관성이 있습니다" → 사용 금지! (너무 자주 쓰임)
   • "~와 관련이 있습니다" → 사용 금지!
   • "~에 영향을 미칠 수 있습니다" → 사용 금지!
   • "~로 인해 발생할 수 있습니다" → 사용 금지!
   • "~에 의해 유발될 수 있습니다" → 사용 금지!
   • "~에 대해" → "~를" 또는 생략
   • "~에 있어서" → "~에서" 또는 생략
   • "~와 같은" → "~처럼" 또는 생략
   
   ✅ 자연스러운 대체 표현:
   • "목 주변 근육이 긴장하면 불편감이 나타납니다"
   • "장시간 같은 자세를 유지하면 목이 뻐근해집니다"
   • "스트레스가 쌓이면 어깨가 굳어집니다"
   • "바람을 쐬면 목이 시린 느낌이 듭니다"

3. 7대 수정 원칙을 반드시 적용:
   - 치료·개선·관리·교정·효과·호전·회복 표현 완전 제거
   - 🚨🚨🚨 치료 효과·호전 기대 표현 절대 금지:
     • "치료 효과를 기대할 수 있습니다" ❌
     • "증상 호전이 가능합니다" ❌
     • "빠른 회복에 도움이 됩니다" ❌
     • "효과적인 치료 방법" ❌
...
6. 자연스럽고 읽기 쉬운 문장으로 수정
   - 번역투 표현을 완전히 제거하고 구어체에 가까운 자연스러운 문장 사용
   - "연관성", "관련이" 같은 딱딱한 표현 대신 "~하면", "~이면" 등 사용
