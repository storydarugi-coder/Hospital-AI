...
🚨 중요: ${Math.floor(targetLength * 1.03)}자 넘으면 안 됨! (예: 2000자 목표 → 2060자 초과 금지!)
  `;
  ...
  /* 기존 상세 프롬프트 주석 처리 (API 타임아웃 방지)
    ...
    [🚨 글자 수 - 가장 중요한 요구사항!!! 🚨]
    ⚠️ 목표: 공백 제외 **정확히 ${targetLength}자 근처** 필수!!! ⚠️
    
    [금지] **절대 금지:** ${targetLength}자보다 적게 쓰는 것! (분량 미달은 실패!)
    ✅ **허용 범위:** ${targetLength}자 ~ ${Math.floor(targetLength * 1.03)}자 (상한선 매우 엄격!)
    🎯 **이상적:** ${targetLength}자 ~ ${Math.floor(targetLength * 1.01)}자 (목표치에 최대한 근접!)
    
    📌 **분량 제한 규칙 (공백 제외):**
    - 1500자 설정 → 1500~1545자 범위 (1545자 넘으면 즉시 중단!)
    - 2000자 설정 → 2000~2060자 범위 (2060자 넘으면 즉시 중단!)
    - 2500자 설정 → 2500~2575자 범위 (2575자 넘으면 즉시 중단!)
    - 3000자 설정 → 3000~3090자 범위 (3090자 넘으면 즉시 중단!)
    - 3500자 설정 → 3500~3605자 범위 (3605자 넘으면 즉시 중단!)
    - ⚠️ 상한선 3% 초과 시 즉시 중단하고 마무리!
    - 🚨 예: 2000자 목표면 2060자 초과 절대 금지! (2700자는 너무 많음!)
    
    📏 분량 가이드 (공백 제외 ${targetLength}자 기준):
    ${targetLength >= 5000 ? `
    - 🔴 5000자 = 매우 긴 글!
    ...
    - 결론: 300자 (핵심 요약 + CTA)
    - 해시태그: 10~15개
    - ⚠️ 최대 5150자까지만!` : targetLength >= 4000 ? `
    - 🔴 4000자 = 긴 글!
    - 서론: 350자
    ...
    - 결론: 250자
    - 해시태그: 10개
    - ⚠️ 최대 4120자까지만!` : targetLength >= 3500 ? `
    - 🟡 3500자 = 상당히 긴 글
    - 서론: 320자
    - 본론 섹션: 4개 섹션 (각 섹션 480자)
    - 각 섹션에 상세 설명 + 예시 포함
    - 결론: 220자
    - 해시태그: 10개
    - ⚠️ 최대 3605자까지만!` : targetLength >= 3000 ? `
    - 🟡 3000자 = 중간 길이 글
    - 서론: 280자
    - 본론 섹션: 3~4개 섹션 (각 섹션 430자)
    - 각 섹션에 설명 + 예시 포함
    - 결론: 200자
    - 해시태그: 10개
    - ⚠️ 최대 3090자까지만!` : targetLength >= 2500 ? `
    - 🟢 2500자 = 적당히 긴 글
    - 서론: 220자
    - 본론 섹션: 3개 섹션 (각 섹션 380자)
    - 결론: 150자
    - 해시태그: 10개
    - ⚠️ 최대 2575자까지만!` : targetLength >= 2000 ? `
    - 🟢 2000자 = 적당한 길이 (딱 이 분량만!)
    - 서론: 170자
    - 본론 섹션: 2~3개 섹션 (각 섹션 310자)
    - 결론: 120자
    - 해시태그: 10개
    - ⚠️ 최대 2060자까지만! (2060자 넘으면 실패!)` : targetLength >= 1500 ? `
    - 🟢 1500자 = 짧고 간결한 글 (딱 이 분량만!)
    - 서론: 110자
    - 본론 섹션: 2개 섹션 (각 섹션 260자)
    - 결론: 90자
    - 해시태그: 8~10개
    - ⚠️ 최대 1545자까지만! (1545자 넘으면 실패!)` : `
    - 🟢 적당한 길이
    - 서론: 간결하게
    - 본론: 2~3개 섹션
    - 결론: 짧게
    ...
    - **${Math.floor(targetLength * 1.03)}자보다 많이 쓰는 것 (3% 초과! 즉시 중단!)**
    - 🚨 예시: 2000자 목표 → 2060자 넘으면 안 됨! (2700자는 절대 금지!)
    - 내용 없이 같은 말 반복하기
    - 해시태그로 글자 수 채우기
    - 불필요하게 길게 늘려쓰기
    ...
    🚨 **분량 초과 방지 (매우 중요!):**
    - 최대 ${Math.floor(targetLength * 1.03)}자까지만! (상한선 엄격 준수)
