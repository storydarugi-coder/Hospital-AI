...
**4. 🔄 키워드 대체 표현 가이드 (동일 키워드 과도 반복 금지!)**
- 병명·시술명: 전체 글에서 5회 이하
- 증상명: 전체 글에서 7회 이하
- **대체 표현 활용 (⚠️ 논문·보고서 단어 사용 금지!):**
  - ❌ 증상 → 불편감(AI 단어!), 양상(논문체!), 시사하다(보고서체!)
  - ✅ 증상 → 불편함, 걸리는 느낌, 신경 쓰임, 몸의 신호, 느껴지는 변화
...
**🚨 문장 길이 제한 (형태소 15개 이하!) - 최우선 규칙!**
- 한 문장의 형태소는 **절대 15개를 넘지 않습니다**
- 긴 문장은 반드시 2~3개로 쪼개기!
- 접속사(~고, ~며, ~지만) 사용 시 문장 끊기!

잘못된 예 (형태소 25개):
❌ "구토가 반복되고 물을 마셔도 유지하기 어려운 상태가 이어질 수 있으며 설사가 물처럼 나오면서 탈수로 이어질 위험도 있습니다."

올바른 예 (각 문장 형태소 12~14개):
✅ "구토가 반복될 수 있습니다. 물을 마셔도 유지하기 어려운 상태가 이어집니다. 설사가 물처럼 나오면서 탈수로 이어질 위험도 있습니다."

**형태소 계산 예시:**
- "구토가 / 반복 / 될 / 수 / 있 / 습니다" = 6개 (✅ 적절)
- "물 / 을 / 마시 / 어도 / 유지 / 하 / 기 / 어렵 / 은 / 상태 / 가 / 이어 / 집니다" = 13개 (✅ 적절)
- "설사 / 가 / 물 / 처럼 / 나오 / 면서 / 탈수 / 로 / 이어 / 질 / 위험 / 도 / 있 / 습니다" = 14개 (✅ 적절)
...
1. **🔍 검색자가 "정보가 있다"고 판단할 수 있어야 한다**
   - ❌ 일기나 에세이 제목: "출근길에 기침 참느라 눈물 뺀 경험", "밤새 뒤척였던 날"
   - ✅ 정보 예고 신호 포함:
     * "~확인해볼 것", "~체크할 부분", "~알아두면 좋은 점"
     * "~인 경우", "~일 때", "~라면"
   - 🎯 **[증상/상황] + [정보가 있다는 신호]** 공식!
     * ✅ "열 없이 기침만 오래갈 때 체크할 부분"
     * ✅ "감기약 먹어도 안 나을 때 의심해볼 수 있는 것"
     * ✅ "기침만 3주째라면 확인해볼 것"
   - 📏 제목 길이: **25자 내외** (${postType === 'card_news' ? '15~25자' : '23~28자'})
...
4. **병명은 제목에 0~1회 (가능하면 0회)**
   - 증상 기반으로 쓰되, 정보 예고를 함께 넣는다
   ❌ "마이코플라스마 폐렴 증상과 치료" (병명 중심)
   ✅ "기침만 3주째라면 확인해볼 것" (증상 중심 + 정보 예고)
   ✅ "열 없이 기침만 오래갈 때 체크할 부분" (증상 + 정보 신호)

5. **같은 구조가 2개 이상 반복되지 않게 한다**
   - 5개 제목이 모두 다른 문장 구조여야 함!
   ❌ "~할 때", "~할 때", "~할 때" (반복)
   ✅ "~할 때" / "~한 뒤로" / "~해서 ~한 경험" / "~하다가" (다양)

6. **"그래서 어떻게 됐는데?" 궁금하게 만든다**
   - 읽는 사람이 본문을 클릭하고 싶어지는 제목
   ❌ "어깨 통증 총정리" (정보 나열 예고)
   ✅ "팔을 올리려는데 갑자기 멈춘 그 순간" (궁금증 유발)

7. **광고가 아니라 경험담·정보글처럼 느껴진다**
   - 병원 안내문/홍보 느낌 단어 금지
   ❌ 관리, 이해, 특징, 연관, 원인, 증상, 치료, 예방, 진단
   ✅ 대체어: 신호, 때, 경우, 순간, 뒤, 느낌, 경험
   ✅ 정보 신호어: 확인해볼 것, 체크할 부분, 알아두면 좋은 점

8. **SEO 최적화**
   - 메인 키워드는 제목 앞 50% 안에 배치 (네이버 SEO 기준)
   - 제목 길이: 25자 내외 (${lengthGuide})
...
[✅ 좋은 제목 예시 - 정보 신호 포함!]
- "열 없이 기침만 오래갈 때 체크할 부분" (증상 + 정보 신호)
- "감기약 먹어도 안 나을 때 의심해볼 수 있는 것" (상황 + 정보 신호)
- "기침만 3주째라면 확인해볼 것" (증상 + 기간 + 정보 신호)
- "아침마다 손가락이 뻣뻣한데, 움직이면 괜찮아지는 경우" (구체적 장면 + 정보)
- "밤에 누우면 어깨가 쑤셔서 옆으로 못 누울 때 알아둘 점" (시간 + 증상 + 정보 신호)
- "세수하다가 팔이 갑자기 안 올라간다면 체크할 부분" (행동 + 증상 + 정보 신호)
- "출근길에 계단 내려가다 무릎이 시큰할 때 확인해볼 것" (장소 + 행동 + 정보 신호)

[❌ 나쁜 제목 예시 - 일기/에세이체, 정보 신호 없음]
...
반드시 JSON 형식으로 응답하고, 목표 길이(공백 제외 ${targetLength}자)를 준수해주세요.
  ...
  /* 기존 상세 프롬프트 주석 처리 (API 타임아웃 방지)
    ...
    [금지] **절대 금지:** ${targetLength}자보다 적게 쓰는 것! (분량 미달은 실패!)
    ✅ **허용 범위:** ${targetLength}자 ~ ${targetLength <= 1500 ? targetLength + 100 : targetLength + 200}자 (엄격한 상한선!)
    🎯 **이상적:** ${targetLength}자 ~ ${targetLength <= 1500 ? targetLength + 50 : targetLength + 100}자
    
    📌 **분량 제한 규칙:**
    - 1500자 설정 → 1500~1600자 (최대 1600자!)
    - 2000자 설정 → 2000~2200자 (최대 2200자!)
    - ⚠️ 상한선 초과 시 즉시 중단하고 마무리!
    ...
    - **${targetLength <= 1500 ? targetLength + 100 : targetLength + 200}자보다 많이 쓰는 것 (분량 초과! 즉시 중단!)**
    - 내용 없이 같은 말 반복하기
    - 해시태그로 글자 수 채우기
    - 불필요하게 길게 늘려쓰기
    
    🚨 **분량 초과 방지:**
    - ${targetLength <= 1500 ? targetLength + 100 : targetLength + 200}자 근처에서 자연스럽게 마무리
