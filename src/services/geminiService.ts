...
2. **판단·진단·결론을 제목에서 내리지 않는다**
   - 제목은 질병을 결론처럼 말하지 않고, 증상이 나타나는 상황에서 멈춘다
   ❌ "~하면 OO일 수 있다" (병명이 결론)
   ❌ "~이면 OO을 의심해봐야" (진단 유도)
   ✅ "~한 증상이 반복될 때" (상황에서 멈춤)
   ✅ "~해서 찝찝했던 경험" (경험 서술)

3. **같은 구조가 2개 이상 반복되지 않게 한다**
   - 5개 제목이 모두 다른 문장 구조여야 함!
   ❌ "~할 때", "~할 때", "~할 때" (반복)
   ✅ "~할 때" / "~한 뒤로" / "~해서 ~한 경험" / "~하다가" (다양)

4. **"그래서 어떻게 됐는데?" 궁금하게 만든다**
   - 읽는 사람이 본문을 클릭하고 싶어지는 제목
   ❌ "어깨 통증 총정리" (정보 나열 예고)
   ✅ "팔을 올리려는데 갑자기 멈춘 그 순간" (궁금증 유발)

5. **광고가 아니라 경험담·정보글처럼 느껴진다**
   - 병원 안내문/홍보 느낌 단어 금지
   ❌ 관리, 이해, 특징, 연관, 원인, 증상, 치료, 예방, 진단
   ✅ 대체어: 신호, 때, 경우, 순간, 뒤, 느낌, 경험

6. **병명은 보조 역할만! 주인공은 증상/상황**
   ❌ "오십견의 특징과 관리" → 병명이 주인공
   ✅ "팔이 귀에 안 닿기 시작할 때" → 증상이 주인공

7. **SEO 최적화**
   - 메인 키워드는 제목 앞 50% 안에 배치 (네이버 SEO 기준)
   - 제목 길이: ${lengthGuide}
...
✅ "스키장에서 넘어진 뒤 무릎에서 뚝 소리가 났는데, 붓기가 안 빠질 때"

❌ "겨울철 운동 중 무릎이 꺾이며 다리에 힘이 들어가지 않는 시점"
✅ "운동하다 무릎이 꺾이고 다리에 힘이 쭉 빠지는 느낌"

❌ "전방십자인대 파열 관련 내용이 다루어지는 상황"
✅ "무릎 다친 뒤 걸을 때마다 휘청거리는 경험"

**AI 연결어 제거:**
❌ "~이 다루어지는", "~에서 설명되는", "~관련 내용이", "~에 대해"
✅ 자연스러운 대화체 연결: "~한 뒤", "~해서", "~하다가"
...
[✅ 좋은 제목 예시]
- "아침마다 손가락이 뻣뻣한데, 움직이면 괜찮아지는 이유"
- "밤에 누우면 어깨가 쑤셔서 옆으로 못 누울 때"
- "세수하다가 팔이 갑자기 안 올라간 그 순간"
- "출근길에 계단 내려가다 무릎이 시큰해서 멈춘 경험"
- "겨울만 되면 손끝이 하얗게 변하는데 따뜻해지면 돌아올 때"

[검수 기준]
- 병·의원 블로그에 게시해도 장기간 노출 가능해야 함
- 의료 광고로 오해받을 표현이 없어야 함
- 독자가 '정보 글' 또는 '경험담'으로 인식할 수 있어야 함
- 🤖 AI 냄새 체크: "흐름/상황/시점/사례/과정/포인트"로 끝나면 탈락!
- 📐 구조 반복 체크: 5개 제목 중 같은 문장 구조 2개 이상이면 탈락!`;
      ...
      responseSchema: {
        ...
        items: {
          ...
          properties: {
            ...
            score: { type: Type.NUMBER },
            type: { type: Type.STRING, enum: ['호기심', '공감', '정보전달', '시기고정', '경험담'] }
          },
