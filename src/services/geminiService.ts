...
████████████████████████████████████████████████████████████████████████████████
[🚨 AI 냄새 6대 패턴 상세 가이드 - 이것만 피하면 된다!]
████████████████████████████████████████████████████████████████████████████████

**⚠️ 패턴 ① 요약 리스트 중복 반복**
문제: 각 문단 끝에 본문 내용을 그대로 재반복하는 요약 리스트
❌ 피해야 할 예:
- 본문에서 "A, B, C 증상이 있습니다" 설명 후
- 바로 아래에 "✓ A 증상 ✓ B 증상 ✓ C 증상" 리스트 반복
✅ 해결법:
- 요약 리스트가 본문과 중복되면 과감히 삭제
- 또는 본문 내용에 자연스럽게 녹여서 서술형으로 변경
- 리스트는 "새로운 정보"를 줄 때만 사용

**⚠️ 패턴 ② 판단 회피형 글쓰기**
문제: '단정하기는 어렵습니다', '확정할 수는 없습니다' 등 회피 표현 3회 이상 반복
❌ 피해야 할 예:
- "단정하기는 어렵습니다"
- "확정할 수는 없습니다"
- "애매해질 수 있다"
- "신경이 쓰인다"
✅ 해결법:
- '단정하기는 어렵습니다' → '이럴 때는 탈수 여부를 먼저 체크하는 것이 중요해요'
- '확정할 수는 없습니다' → '여러 가능성이 있지만, 현재 가장 불편한 증상에 집중해볼까요?'
- 부정적 회피 표현(~어렵습니다)보다 긍정적 행동 제안(~해보는 게 좋아요)으로!

**⚠️ 패턴 ③ 현장감 부재 (논문 요약 느낌)**
문제: '질병관리청 자료', '대한감염학회', '대한내과학회' 등 기관명 5회 이상 언급
❌ 피해야 할 예:
- "질병관리청 자료에 따르면..."
- "대한소화기학회 가이드라인에서는..."
- "대한내과학회 권고사항에 의하면..."
✅ 해결법:
- 기관명 언급 → '최근 건강 지침에 따르면', '일반적으로 권고되는 사항은'으로 순화
- 교과서적 설명 → 일상 언어 사용 ('겨울철 생굴은 참 맛있지만, 뒤탈이 나면 정말 고생스럽죠')
- 구체적 정황 묘사 추가 ('물만 마셔도 10분 뒤에 신호가 와서 겁부터 나는 상황이라면')

**⚠️ 패턴 ④ 가짜 공감 (기계적 공감)**
문제: 3인칭 관찰자 시점의 기계적 공감 표현
❌ 피해야 할 예:
- "비슷한 고민을 가진 분들이 가장 많이 묻는 건 이거예요" (기계적 도입)
- "마음이 편하다는 이야기"를 들었다 (3인칭 관찰)
✅ 해결법:
- '가장 많이 묻는 건 이거예요' → '저도 굴 먹고 배탈 났을 때 이 부분이 제일 헷갈리더라고요' (1인칭 경험)
...
❌ 피해야 할 예:
- "객관적인 기준으로 상태를 확인하는 쪽이 먼저"
- "건강한 습관을 유지하시기 바랍니다"
✅ 해결법:
- 추상적 → 구체적: '탈수 증상이 지속된다면 전문가의 도움을 받아보시는 것이 안전해요' (의료법 준수)
- CTA 박스에 집에서 할 수 있는 구체적 행동 리스트 추가:
  · "체온 체크해보기"
  · "수분 섭취량 체크해보기"
  · "증상 시작 시간 메모해두기"

**⚠️ 패턴 ⑥ 과도한 설명 (TMI)**
문제: 모든 가능성을 나열하고 모든 걸 설명하려 함
❌ 피해야 할 예:
- "A일 수도 있고, B일 수도 있고, C일 수도 있고, D일 수도 있습니다. A는 ~이고..."
✅ 해결법:
- 대표적인 것 2-3개만 언급
- "물론 사람마다 다르겠지만요" 같은 열린 표현으로 여백 남기기
- 나머지는 "이 외에도 여러 원인이 있을 수 있어요"로 정리

**[🎯 6대 패턴 자가 체크리스트]**
□ 요약 리스트가 본문 내용과 중복되지 않는가?
□ '어렵습니다/없습니다' 회피 표현이 3회 이상 나오지 않는가?
□ 기관명 언급이 5회 이상 나오지 않는가?
□ 공감 표현이 1인칭 경험 기반인가? (3인칭 관찰 X)
□ CTA가 구체적 행동을 제시하는가? (추상적 마음가짐 X)
□ 모든 가능성을 나열하지 않고 여백을 남겼는가?

**→ 6개 모두 OK면 = 사람이 쓴 글처럼 읽힌다!**
