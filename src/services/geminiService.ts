...
1. **질환명/증상 키워드 자연스럽게 포함**
   - 제목 맨 앞에 고정하지 말고 문맥 속에 자연스럽게 녹인다
   - 의학 용어보다 일상 표현을 우선, 필요할 때만 병명 사용
   
   ✅ "무릎에서 소리 나고 붓기가 안 빠질 때 살펴볼 점"
   ✅ "열 없이 기침만 오래갈 때 알아둘 것"
   ✅ "아침마다 손가락이 뻣뻣한 경우"

2. **'증상/상황 + 궁금증' 구조 기본**
   - 실제 사용자가 검색창에 입력할 법한 말투 사용
   - 질문형 또는 조건형 문장 우선
   
   ✅ "~일 때", "~인 경우", "~느껴질 때", "~달라졌다면"
   ✅ "계단 내려가다 무릎에서 소리 난 뒤로 걸을 때마다 불편하다면"
   ✅ "출근길 지하철에서 손잡이 잡기가 힘들 때"
...
3. **정보 탐색형 표현 활용**
   ...
   ✅ "무릎 통증이 생활 변화와 관련 있을 때"
   ✅ "기침이 시간이 지나도 나아지지 않는 경우"
   ✅ "어깨 불편함이 정상 범위와 다른 신호일 때"

4. **한 제목에 핵심 키워드 1~2개만 사용**
   - 제목 길이: 25~40자 이내로 자연스럽게
   
   ❌ "무릎 통증 원인 증상 치료 방법" (키워드 나열)
   ✅ "무릎 통증이 오래갈 때 살펴볼 점" (자연스러운 문장)
...
6. **같은 구조가 2개 이상 반복되지 않게**
   - 5개 제목이 모두 다른 문장 구조여야 함!
   ❌ "~할 때", "~할 때", "~할 때" (반복)
   ✅ "~할 때" / "~한 뒤로" / "~해서" / "~하다가" / "~라면" (다양)

7. **구체적 상황 제시**
   - 시간대, 장소, 행동 중 하나가 구체적으로 드러나야 함
   
   ❌ "어깨 통증이 있을 때" (추상적)
   ✅ "아침에 세수하다가 팔이 안 올라갈 때" (구체적)
...
❌ 숫자: 모든 수치 표현 금지
   • 통계: "90%", "절반", "대부분의"
   • 기간: "2주", "3개월", "며칠", "하루", "1년"
   • 연령: "50대", "30대", "40대" 등 숫자 포함 (⚠️ "중년", "노년", "중년 이후"는 허용)
   • 횟수: "3회", "몇 번", "여러 차례"
   • 시간: "10분", "30분", "하루"
...
❌ 의료용어: "의료", "의료진", "전문의", "의사", "진단", "처방", "검진", "진료", "증상"(추상어)
...
⚠️ 중요: 개인 경험, 에피소드 금지
   ...
   ✅ 현대인들이 겪을 법한 구체적 상황 (일반적 시나리오)
   ✅ 관찰자 시점, 정보 제공 중심으로 작성

[✅ 네이버 SEO 최적화]
1. 제목 전략:
   ⚠️ 실제 그 질환을 가진 사람들이 검색해 볼 법한 제목
